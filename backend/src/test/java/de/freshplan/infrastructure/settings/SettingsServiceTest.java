...
class SettingsServiceTest {
  ...
  void testCreateAndRetrieveGlobalSetting() {
    // Given
    JsonObject value = new JsonObject().put("enabled", true).put("threshold", 100);
    JsonObject metadata = new JsonObject().put("description", "Test setting");

    // When
    Setting created =
        settingsService.saveSetting(
            SettingsScope.GLOBAL, null, TEST_KEY, value, metadata, TEST_USER);
    ...
    // Verify retrieval
    Optional<Setting> retrieved = settingsService.getSetting(SettingsScope.GLOBAL, null, TEST_KEY);
  ...
  void testHierarchicalResolution() {
    ...
    settingsService.saveSetting(SettingsScope.GLOBAL, null, TEST_KEY, globalValue, null, TEST_USER);
    settingsService.saveSetting(
        SettingsScope.TENANT, TENANT_ID, TEST_KEY, tenantValue, null, TEST_USER);
    settingsService.saveSetting(
        SettingsScope.TERRITORY, TERRITORY_DE, TEST_KEY, territoryValue, null, TEST_USER);
    settingsService.saveSetting(
        SettingsScope.ACCOUNT, ACCOUNT_ID, TEST_KEY, accountValue, null, TEST_USER);

    // When - Resolve with full context (should get account-level)
    var fullContext =
        new SettingsService.SettingsContext(TENANT_ID, TERRITORY_DE, ACCOUNT_ID, null);
    ...
    var territoryContext = new SettingsService.SettingsContext(TENANT_ID, TERRITORY_DE, null, null);
  ...
  void testEtagOptimisticLocking() {
    ...
    Setting created =
        settingsService.saveSetting(
            SettingsScope.GLOBAL, null, TEST_KEY, initialValue, null, TEST_USER);
    ...
    Setting updated =
        settingsService.updateSettingWithEtag(id, updatedValue, null, originalEtag, TEST_USER);
    ...
    WebApplicationException ex412 =
        assertThrows(
            WebApplicationException.class,
            () ->
                settingsService.updateSettingWithEtag(
                    id, failedValue, null, originalEtag, TEST_USER));
    assertEquals(412, ex412.getResponse().getStatus());

    // When/Then - Update without ETag should fail (428 Precondition Required)
    WebApplicationException ex428 =
        assertThrows(
            WebApplicationException.class,
  ...
  void testEtagCheck() {
    ...
    Setting created =
        settingsService.saveSetting(SettingsScope.GLOBAL, null, TEST_KEY, value, null, TEST_USER);
  ...
  void testListSettings() {
    ...
    settingsService.saveSetting(
        SettingsScope.TENANT, TENANT_ID, "setting1", new JsonObject(), null, TEST_USER);
    settingsService.saveSetting(
        SettingsScope.TENANT, TENANT_ID, "setting2", new JsonObject(), null, TEST_USER);
    settingsService.saveSetting(
        SettingsScope.TERRITORY, TERRITORY_DE, "setting3", new JsonObject(), null, TEST_USER);

    // When
    List<Setting> tenantSettings = settingsService.listSettings(SettingsScope.TENANT, TENANT_ID);
    List<Setting> territorySettings =
        settingsService.listSettings(SettingsScope.TERRITORY, TERRITORY_DE);
  ...
  void testDeleteSetting() {
    ...
    Setting created =
        settingsService.saveSetting(
            SettingsScope.GLOBAL, null, TEST_KEY, new JsonObject(), null, TEST_USER);
  ...
  void testTerritoryIsolation() {
    ...
    JsonObject deValue = new JsonObject().put("currency", "EUR").put("vatRate", 19);
    JsonObject chValue = new JsonObject().put("currency", "CHF").put("vatRate", 7.7);

    settingsService.saveSetting(
        SettingsScope.TERRITORY, TERRITORY_DE, "tax.config", deValue, null, TEST_USER);
    settingsService.saveSetting(
        SettingsScope.TERRITORY, TERRITORY_CH, "tax.config", chValue, null, TEST_USER);

    // When - Resolve for DE context
    var deContext = new SettingsService.SettingsContext(TENANT_ID, TERRITORY_DE, null, null);
    ...
    var chContext = new SettingsService.SettingsContext(TENANT_ID, TERRITORY_CH, null, null);
  ...
  void testCreateRaceCondition() {
    ...
    Setting first =
        settingsService.createSettingStrict(
            SettingsScope.GLOBAL, null, "race.test", value, null, TEST_USER);
