...
/**
 * Tests for EventSubscriber reconnection handling.
 * Verifies that the subscriber properly handles connection failures and reconnects.
 */
@QuarkusTest
public class EventSubscriberReconnectTest {
    ...
    public void testReconnectAfterConnectionLoss() throws Exception {
        ...
        }).when(mockConnection).isValid(anyInt());
        ...
        assertTrue(connectionCheckLatch.await(5, TimeUnit.SECONDS),
    ...
    public void testGucVariablesSetAfterReconnect() throws Exception {
        ...
        verify(mockStatement, timeout(1000).atLeastOnce())
            .execute(contains("SET app.current_user"));
        verify(mockStatement, timeout(1000).atLeastOnce())
            .execute(contains("SET app.current_role"));
    }
    ...
    public void testSystemUserUsedForEventListener() throws Exception {
        ...
        verify(mockStatement, timeout(1000).atLeastOnce())
            .execute(contains("events-bus@freshplan"));
        verify(mockStatement, timeout(1000).atLeastOnce())
            .execute(contains("SYSTEM"));
    }
