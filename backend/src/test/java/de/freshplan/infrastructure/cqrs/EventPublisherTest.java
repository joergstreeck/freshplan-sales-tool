...
/**
 * Test class for CQRS Light Event Publisher
 * Validates event publishing with <200ms performance target
 */
@QuarkusTest
public class EventPublisherTest {
    ...
    void cleanup() {
        ...
        try (Connection conn = dataSource.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                 "DELETE FROM domain_events WHERE user_id = 'test-user'")) {
            stmt.executeUpdate();
        } catch (Exception e) {
    ...
    void testPublishEvent() {
        ...
        EventPublisher.DomainEvent event = EventPublisher.DomainEvent.builder()
    ...
    void testPublishCorrelatedEvents() {
        ...
        EventPublisher.DomainEvent leadEvent = EventPublisher.DomainEvent.builder()
        ...
        EventPublisher.DomainEvent customerEvent = EventPublisher.DomainEvent.builder()
    ...
    void testValidateEventType() {
        // Arrange
        EventPublisher.DomainEvent invalidEvent = EventPublisher.DomainEvent.builder()
    ...
    void testEventPerformance() {
        ...
        assertTrue(p95 < 200,
            String.format("P95 latency should be <200ms, was %dms", p95));
    }

    private void verifyEventInDatabase(UUID eventId, String eventType,
                                       UUID aggregateId) {
        try (Connection conn = dataSource.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                assertNotNull(rs.getObject("aggregate_id"),
                    "Event should have aggregate_id");
            }
        } catch (Exception e) {
    ...
    private void verifyEventCorrelation(UUID event1Id, UUID event2Id,
                                       UUID correlationId) {
        try (Connection conn = dataSource.getConnection();
             PreparedStatement stmt = conn.prepareStatement(
                 "SELECT correlation_id FROM domain_events WHERE id IN (?, ?)")) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                int count = 0;
                while (rs.next()) {
                    assertEquals(correlationId, rs.getObject("correlation_id"),
