 ...
 *
 * <p>Tests migration integrity and idempotency: - All migrations are applied successfully - No
 * pending migrations - No failed migrations - Migration checksums are valid - Migrations are
 * idempotent (repeatable without errors) - Migration order is correct
 *
 * <p>WHY: Prevents production deployment with broken migrations BLOCKING: CI fails if migrations
 * are in invalid state
 *
 * <p>NOTE: Quarkus Flyway Community Edition does NOT support flyway:undo (Teams/Enterprise only).
...
public class FlywayMigrationValidationTest {
  ...
  void testAllMigrationsApplied() {
    ...
    if (!pending.isEmpty()) {
      String pendingList =
          pending.stream()
              .map(m -> String.format("  - %s: %s", m.getVersion(), m.getDescription()))
              .collect(Collectors.joining("\n"));
  ...
  void testMigrationChecksumsValid() {
    ...
    if (!outdated.isEmpty()) {
      ...
      fail(
          String.format(
              "Found %d migrations with checksum mismatches:\n%s\n\n"
  ...
  void testMigrationOrderCorrect() {
    ...
    if (outOfOrder.size() > 5) {
      ...
      outOfOrder.forEach(
          m -> System.err.println("  - " + m.getVersion() + ": " + m.getDescription()));
      System.err.println(
    ...
    try (Connection conn = dataSource.getConnection()) {
      for (String table : criticalTables) {
        try (PreparedStatement stmt =
            conn.prepareStatement("SELECT to_regclass('public.' || ?) IS NOT NULL")) {
          ...
          try (ResultSet rs = stmt.executeQuery()) {
            ...
            assertTrue(
                exists,
                String.format(
                    "Critical table '%s' does not exist after migrations.\n"
                        + "This indicates incomplete or failed migrations.",
                    table));
          }
        }
      }
  ...
  void testMigrationsAreIdempotent() {
    ...
    assertEquals(
        ...
        appliedCount,
        String.format(
            "Expected 0 new migrations (idempotent), but %d were applied.\n"
                + "This indicates migrations are not idempotent or pending migrations exist.\n"
                + "Fix: Ensure all migrations use IF NOT EXISTS / IF EXISTS checks.",
            appliedCount));
  }
