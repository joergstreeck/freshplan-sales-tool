 ...
 * security contracts required for the ABAC/RLS security foundation.
 *
 * <p>Part of FP-231: Security Gates Enforcement.
 */
@QuarkusTest
...
public class SecurityContractTest {
  ...
  /**
   * Security Contract 1: Owner Access Test Lead creator must be able to read and edit their own
   * leads.
   */
  @Test
  ...
  void testOwnerAccessContract() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      if (tableExists(conn, "demo_security_items")) {
        ...
        ps = conn.prepareStatement("SELECT COUNT(*) FROM demo_security_items WHERE id = ?");
  ...
  /**
   * Security Contract 2: Collaborator Access Test Team members in same territory can read but not
   * edit.
   */
  @Test
  ...
  void testCollaboratorAccessContract() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      if (itemId != null && tableExists(conn, "demo_security_items")) {
        ...
        rs = ps.executeQuery();
        assertTrue(rs.next());
        assertTrue(rs.getInt(1) > 0, "Collaborator should see items in same territory");
  ...
  /** Security Contract 3: Manager Override Test Managers can override with audit trail. */
  @Test
  ...
  void testManagerOverrideContract() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      if (tableExists(conn, "demo_security_items")) {
        ps = conn.prepareStatement("SELECT COUNT(*) >= 0 FROM demo_security_items");
        rs = ps.executeQuery();
        assertTrue(rs.next());
        assertTrue(rs.getBoolean(1), "Manager with admin role should access items");
      }

      // Verify audit log table exists (for audit trail)
      assertTrue(
          tableExists(conn, "security_audit_log"),
          "Audit log table should exist for tracking overrides");
    }
  }

  /** Security Contract 4: Territory Isolation Test DE users cannot see CH leads and vice versa. */
  @Test
  ...
  /** Security Contract 5: Fail-Closed Test Policy errors must block access (fail closed). */
  @Test
  ...
  void testFailClosedContract() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      rs = ps.executeQuery();
      assertTrue(rs.next());
      assertFalse(rs.getBoolean(1), "Without context, has_role should fail closed (return false)");

      // Test invalid UUID handling
      ps = conn.prepareStatement("SELECT set_app_context('invalid-uuid'::uuid, ?, ?, NULL)");
      ps.setString(1, ORG_FRESHFOODZ);
      ps.setString(2, "DE");
      ...
      try {
        ...
        assertTrue(
            e.getMessage().contains("uuid") || e.getMessage().contains("UUID"),
