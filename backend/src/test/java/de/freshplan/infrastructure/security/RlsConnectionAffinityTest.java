...
/**
 * Test RLS Connection Affinity for infrastructure services.
 * Verifies that GUC variables are properly set on the same connection
...
public class RlsConnectionAffinityTest {
    ...
    void testGucVariablesSetOnConnection() {
        ...
        String actualUser = (String) em.createNativeQuery(
    ...
    void testTerritoryIsolation() {
        ...
        Long deCount = (Long) em.createNativeQuery(
            "SELECT COUNT(*) FROM settings WHERE key = 'test_de'"
        ).getSingleResult();

        Long chCount = (Long) em.createNativeQuery(
            "SELECT COUNT(*) FROM settings WHERE key = 'test_ch'"
        ).getSingleResult();
    ...
    void testAdminBypassesRls() {
        ...
        Long totalCount = (Long) em.createNativeQuery(
            "SELECT COUNT(*) FROM settings WHERE key LIKE 'test_admin_%'"
        ).getSingleResult();
    ...
    void testConnectionAffinityMaintained() {
        ...
        String user1 = (String) em.createNativeQuery(
            "SELECT current_setting('app.current_user', true)"
        ).getSingleResult();
        ...
        String user2 = (String) em.createNativeQuery(
            "SELECT current_setting('app.current_user', true)"
        ).getSingleResult();
