 ...
 * 
 * Tests the CQRS-separated AuditCommandService and AuditQueryService
 * with Feature Flag enabled to ensure proper delegation and functionality.
 * 
 * Key aspects tested:
 * - Async audit logging via CommandService
 * - Query operations via QueryService  
...
class AuditCQRSIntegrationTest {
    ...
    @Inject
    AuditService auditService; // Test via Facade to verify Feature Flag switching
    
    @Inject
    AuditRepository auditRepository; // For verification
    ...
    void logSecurityEvent_inCQRSMode_shouldCreateEntry() {
        ...
        UUID auditId = auditService.logSecurityEvent(
            ...
            "User testuser logged in from 192.168.1.1"
        );
    ...
    void findByEntity_inCQRSMode_shouldReturnEntries() {
        ...
        AuditContext context1 = AuditContext.builder()
            .eventType(AuditEventType.CUSTOMER_CREATED)
            .entityType("customer")
            .entityId(entityId)
            .userName("testuser")
        ...
        AuditContext context2 = AuditContext.builder()
        ...
        assertThat(trail)
            .allMatch(entry -> entry.getEntityId().equals(entityId));
    }
            ...
            .until(() -> {
                ...
                return QuarkusTransaction.call(() -> {
                    ...
                    return !trail.isEmpty() && 
                           trail.get(0).getNewValue().contains(uniqueValue);
                });
            });
    }
    ...
    void logAsync_withInvalidContext_shouldHandleError() {
        ...
        AuditContext validContext = AuditContext.builder()
            .eventType(AuditEventType.CUSTOMER_CREATED)
            .entityType("customer")
            .entityId(UUID.randomUUID())
            .userName("testuser")
        ...
        try {
            ...
            AuditContext invalidContext = AuditContext.builder()
    ...
    void cqrsMode_shouldProperlyDelegateAllOperations() {
        ...
        AuditContext createContext = AuditContext.builder()
            .eventType(AuditEventType.CUSTOMER_CREATED)
            .entityType("customer")
            .entityId(entityId)
            .userName("testuser")
        ...
        AuditContext updateContext = AuditContext.builder()
            .eventType(AuditEventType.CUSTOMER_UPDATED)
            .entityType("customer")
            .entityId(entityId)
            .userName("testuser")
