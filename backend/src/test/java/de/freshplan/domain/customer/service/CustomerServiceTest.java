...
/**
 * Comprehensive unit tests for CustomerService. Tests all business logic and service methods with
 * proper mocking.
 *
 ...
 */
@QuarkusTest
@TestSecurity(user = "testuser", roles = {"admin", "manager", "sales", "viewer"})@DisplayName("CustomerService Tests")
class CustomerServiceTest {
  ...
  class CustomerUpdate {
    ...
    void updateCustomer_withValidData_shouldReturnUpdatedCustomer() {
      ...
      CustomerResponse result =
          customerService.updateCustomer(customerId, validUpdateRequest, "test-user");
  ...
  class Dashboard {
    ...
    void getDashboard_shouldReturnCorrectData() {
      ...
      when(customerRepository.countByLifecycleStage(CustomerLifecycleStage.ACQUISITION))
          .thenReturn(55L);
      when(customerRepository.countByLifecycleStage(CustomerLifecycleStage.ONBOARDING))
          .thenReturn(20L);
      when(customerRepository.countByLifecycleStage(CustomerLifecycleStage.GROWTH)).thenReturn(60L);
      when(customerRepository.countByLifecycleStage(CustomerLifecycleStage.RETENTION))
          .thenReturn(10L);
      when(customerRepository.countByLifecycleStage(CustomerLifecycleStage.RECOVERY))
          .thenReturn(5L);
      ...
      assertThat(result.customersByLifecycle().get(CustomerLifecycleStage.ACQUISITION))
          .isEqualTo(55L);
