 ...
 * 
 * Tests the CQRS-separated TimelineCommandService and TimelineQueryService
 * with Feature Flag enabled to ensure proper delegation and functionality.
 * 
 * Key aspects tested:
 * - Timeline event creation via CommandService
 * - Timeline event retrieval via QueryService
...
class TimelineCQRSIntegrationTest {
    ...
    void createNote_inCQRSMode_shouldCreateSuccessfully() {
        ...
        CreateNoteRequest noteRequest = new CreateNoteRequest(
            "Important customer feedback about service quality",
            "testuser"
        );
    ...
    void createCommunication_inCQRSMode_shouldCreateSuccessfully() {
        // Given
        CreateCommunicationRequest commRequest = CreateCommunicationRequest.builder()
            .channel("email")
            .direction("outbound")
    ...
    void updateEvent_inCQRSMode_shouldUpdateSuccessfully() {
        ...
        // When
        TimelineEventResponse updated = timelineService.updateEvent(
            created.getId(),
            updateRequest
        );
    ...
    void getRecentCommunications_inCQRSMode_shouldReturnRecentComms() {
        ...
        // When
        List<TimelineEventResponse> recentComms = timelineService.getRecentCommunications(
            customerId,
            7 // last 7 days
        );
    ...
    void createEvent_forNonExistentCustomer_shouldThrowException() {
        ...
        assertThatThrownBy(() -> 
            timelineService.createEvent(nonExistentId, request)
        ).isInstanceOf(de.freshplan.domain.customer.service.exception.CustomerNotFoundException.class)
    ...
    void updateEvent_nonExistent_shouldThrowException() {
        ...
        // When/Then
        assertThatThrownBy(() -> 
            timelineService.updateEvent(nonExistentEventId, request)
