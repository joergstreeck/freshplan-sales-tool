...
public class ApiSecurityE2ETest {
    ...
    public static void setupTokens() {
        try {
            ...
            if (keycloakAvailable) {
                ...
                System.err.println("⚠️  Keycloak is not running! Start it with: ./scripts/start-keycloak.sh");
            }
        } catch (Exception e) {
    ...
    public void skipIfKeycloakNotAvailable() {
        Assumptions.assumeTrue(keycloakAvailable, 
            "Keycloak is not available. Skipping E2E tests.");
    }
    ...
    void testSalesCockpitAccess() {
        ...
        Response adminCockpit = given()
            .header("Authorization", "Bearer " + adminToken)
            .when()
            .get("/api/sales-cockpit");
        assertThat(adminCockpit.getStatusCode()).isEqualTo(200);
        
        Response managerCockpit = given()
            .header("Authorization", "Bearer " + managerToken)
            .when()
            .get("/api/sales-cockpit");
        assertThat(managerCockpit.getStatusCode()).isEqualTo(200);
        
        Response salesCockpit = given()
            .header("Authorization", "Bearer " + salesToken)
            .when()
            .get("/api/sales-cockpit");
