 ...
 *
 * <p>Validiert die strikte Trennung von DE/CH/AT Territories durch RLS. Stellt sicher, dass keine
 * Cross-Territory-Datenlecks möglich sind.
 */
@QuarkusTest
...
public class TerritoryIsolationTest {
  ...
  void testCrossTerritoryProtection() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      PreparedStatement ps =
          conn.prepareStatement("SELECT company_name FROM leads WHERE id = ? AND territory = 'CH'");
      ps.setObject(1, leadCH1.id);

      ResultSet rs = ps.executeQuery();

      if (isRlsEnabled(conn)) {
        assertFalse(rs.next(), "DE user should NOT be able to access CH lead with RLS enabled");
      } else {
  ...
  void testTerritorySwitchPrevention() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      PreparedStatement ps = conn.prepareStatement("SELECT current_app_territory()");
      ResultSet rs = ps.executeQuery();
      assertTrue(rs.next());
      ...
      try {
        ...
        if (isAuthorizationEnabled()) {
          assertEquals("DE", newTerritory, "Unauthorized territory switch should be prevented");
        }
      } catch (SQLException e) {
        // Expected if authorization prevents switch
        assertTrue(
            e.getMessage().contains("authorization") || e.getMessage().contains("permission"),
  ...
  void testMultiTerritoryManagerAccess() throws SQLException {
    ...
    try (Connection conn = dataSource.getConnection()) {
      ...
      if (hasRole(conn, "manager")) {
        assertTrue(territoriesVisible >= 1, "Manager should see at least their primary territory");
      }
    }
  }
  ...
  void testTerritoryBusinessRules() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      PreparedStatement ps =
          conn.prepareStatement("SELECT currency, vat_rate FROM territories WHERE code = 'DE'");
      ResultSet rs = ps.executeQuery();

      if (rs.next()) {
        assertEquals("EUR", rs.getString("currency"), "DE territory should use EUR");
        assertEquals(19.0, rs.getDouble("vat_rate"), 0.01, "DE territory should have 19% VAT");
      }

      // Test CH territory rules (CHF, 7.7% VAT)
      setRlsContext(conn, USER_CH_1, TEST_ORG, "CH", "sales");

      ps = conn.prepareStatement("SELECT currency, vat_rate FROM territories WHERE code = 'CH'");
      rs = ps.executeQuery();

      if (rs.next()) {
        assertEquals("CHF", rs.getString("currency"), "CH territory should use CHF");
        assertEquals(7.7, rs.getDouble("vat_rate"), 0.01, "CH territory should have 7.7% VAT");
      }
    }
  }
  ...
  void testRlsPoliciesActive() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      if (policyCount > 0) {
        assertTrue(policyCount >= 2, "Should have at least SELECT and INSERT policies for leads");
      } else {
      ...
      if (rs.next()) {
        ...
        if (rlsEnabled) {
          assertTrue(
              rlsForced, "RLS should be force-enabled for leads table (FORCE ROW LEVEL SECURITY)");
        }
      }
    }
  }
  ...
  private boolean isRlsEnabled(Connection conn) throws SQLException {
    PreparedStatement ps =
        conn.prepareStatement("SELECT relrowsecurity FROM pg_class WHERE relname = 'leads'");
    ResultSet rs = ps.executeQuery();
  ...
  private boolean isAuthorizationEnabled() {
    ...
    return System.getProperty("security.authorization.enabled", "false").equalsIgnoreCase("true");
  }
}
