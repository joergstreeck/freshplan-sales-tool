 ...
 *
 * <p>Validiert die komplette 6M+60T+10T State-Machine mit allen Übergängen: - 6 Monate initiale
 * Protection - 60 Tage Inaktivitäts-Reminder - 10 Tage Grace Period - Stop-the-Clock Feature -
 * Activity-basierte Verlängerungen
 *
 * <p>Testet die Integration von REST API bis Database mit allen Security-Checks.
 */
@QuarkusTest
...
public class UserLeadProtectionE2ETest {
  ...
  void test60DayInactivityReminder() {
    ...
    assertNotNull(updated.reminderSentAt, "Reminder timestamp should be set");

    // Verify activity was created
    LeadActivity activity =
        LeadActivity.find("lead.id = ?1 AND type = ?2", testLead.id, ActivityType.REMINDER_SENT)
            .firstResult();
  ...
  void test10DayGracePeriod() {
    ...
    assertNotNull(updated.gracePeriodStartAt, "Grace period start timestamp should be set");
  }
  ...
  void testStopTheClockFeature() {
    ...
    Lead updated = Lead.findById(testLead.id);
    assertEquals(
        LeadStatus.ACTIVE, updated.status, "Lead should remain ACTIVE when clock is stopped");
    assertNull(updated.reminderSentAt, "Reminder should NOT be sent when clock is stopped");
  ...
  void testQualifyingActivityResetsTimer() {
    ...
    Lead updated = Lead.findById(testLead.id);
    assertEquals(
        LeadStatus.ACTIVE, updated.status, "Lead should remain ACTIVE after qualifying activity");
    assertNull(
        updated.reminderSentAt, "Reminder should NOT be sent after recent qualifying activity");
  }
  ...
  void testProtectionQueryPerformance() {
    ...
    assertTrue(
        queryTime < 200, String.format("Protection query must be <200ms, was %dms", queryTime));
  }
  ...
  void testUserSettingsIntegration() {
    // Get user settings
    var settings = settingsService.getUserLeadSettings(TEST_USER_ID, TEST_TERRITORY);

    assertNotNull(settings, "User settings should exist");

    // Verify default protection periods
    assertEquals(6, settings.getProtectionMonths(), "Default protection should be 6 months");
    assertEquals(60, settings.getReminderDays(), "Default reminder should be 60 days");
    assertEquals(10, settings.getGracePeriodDays(), "Default grace period should be 10 days");
