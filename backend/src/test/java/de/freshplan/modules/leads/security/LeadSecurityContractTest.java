 ...
 *
 * <p>Diese Tests validieren die ABAC/RLS Security-Integration für Lead-Management: 1. Owner-Access:
 * Lead-Ersteller können ihre eigenen Leads lesen/bearbeiten 2. Territory-Isolation: DE/CH Leads
 * sind strikt getrennt 3. User-Lead-Protection: 6M+60T+10T State-Machine Compliance 4.
 * Collaborator-Access: Team-Mitglieder können Leads im selben Territory lesen 5. Manager-Override:
 * Manager können mit Audit-Trail überschreiben
 *
 * <p>Sprint 2.1 - PR #4: Security-Integration
 */
@QuarkusTest
...
public class LeadSecurityContractTest {
  ...
  /**
   * Security Contract 1: Owner Access Lead-Ersteller muss eigene Leads vollständig verwalten
   * können.
   */
  @Test
  @Order(1)
  ...
  void testOwnerAccessContract() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      PreparedStatement ps =
          conn.prepareStatement("SELECT COUNT(*) FROM leads WHERE id = ? AND owner_user_id = ?");
      ...
      ps = conn.prepareStatement("SELECT protection_expires_at FROM leads WHERE id = ?");
      ps.setObject(1, testLeadDE.id);
      rs = ps.executeQuery();
  ...
  /** Security Contract 2: Territory Isolation DE und CH Leads müssen strikt getrennt sein. */
  @Test
  @Order(2)
  ...
  void testTerritoryIsolationContract() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      PreparedStatement ps =
          conn.prepareStatement("SELECT COUNT(*) FROM leads WHERE territory = 'CH'");
      ResultSet rs = ps.executeQuery();
      assertTrue(rs.next());
      ...
      if (isRlsActive(conn)) {
        assertEquals(0, chLeadsVisible, "DE user should NOT see CH leads when RLS is active");
      } else {
  ...
  /**
   * Security Contract 3: User-Lead-Protection 6 Monate + 60 Tage + 10 Tage State-Machine muss
  ...
  /**
   * Security Contract 4: Collaborator Access Team-Mitglieder im selben Territory können Leads
  ...
  void testCollaboratorAccessContract() throws SQLException {
    try (Connection conn = dataSource.getConnection()) {
      ...
      PreparedStatement ps =
          conn.prepareStatement("SELECT COUNT(*) FROM leads WHERE territory = 'DE'");
      ResultSet rs = ps.executeQuery();
      assertTrue(rs.next());
      assertTrue(rs.getInt(1) > 0, "Collaborator should see leads in same territory");

      // Collaborator should NOT be able to update lead they don't own
      ps = conn.prepareStatement("SELECT owner_user_id FROM leads WHERE id = ?");
      ps.setObject(1, testLeadDE.id);
      rs = ps.executeQuery();

      if (rs.next()) {
        UUID ownerId = (UUID) rs.getObject(1);
        assertNotEquals(USER_COLLABORATOR, ownerId, "Collaborator should not be owner of the lead");
      }

      // Verify read-only access pattern
      ps = conn.prepareStatement("SELECT company_name FROM leads WHERE id = ?");
      ps.setObject(1, testLeadDE.id);
      rs = ps.executeQuery();
  ...
  /**
   * Security Contract 5: Manager Override with Audit Manager können mit Audit-Trail überschreiben.
   */
  @Test
  @Order(5)
  ...
  void testPerformanceContract() {
    ...
    assertTrue(
        queryTime < 200, String.format("Query performance must be <200ms, was %dms", queryTime));
  }

  // Helper methods

  private void setRlsContext(
      Connection conn, UUID userId, String org, String territory, String roles)
      throws SQLException {
    PreparedStatement ps =
        conn.prepareStatement("SELECT set_app_context(?::uuid, ?, ?, string_to_array(?, ','))");
  ...
  private boolean isRlsActive(Connection conn) throws SQLException {
    ...
    PreparedStatement ps =
        conn.prepareStatement("SELECT COUNT(*) FROM pg_policies WHERE tablename = 'leads'");
    ResultSet rs = ps.executeQuery();
    rs.next();
