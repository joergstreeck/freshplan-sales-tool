 ...
 *
 * Validiert, dass alle Lead-Operations die Performance-SLOs einhalten:
 * - API Response: <200ms P95
 * - Database Queries: <50ms P95
 * - Bulk Operations: Linear scaling
 * - Concurrent Access: No degradation
 *
 * Sprint 2.1 - Performance Validation
 */
@QuarkusTest
...
public class LeadPerformanceTest {
    ...
    void testLeadListQueryPerformance() {
        ...
        for (int pageSize : pageSizes) {
            ...
            assertTrue(p95 < P95_THRESHOLD_MS,
                String.format("Lead list (size=%d) P95 should be <200ms, was %dms",
                    pageSize, p95));
        }
    }
    ...
    void testDatabaseQueryPerformance() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            startTime = System.nanoTime();
            List<Lead> complexQuery = Lead.find(
                "territory = ?1 AND status = ?2 AND protectionExpiresAt > ?3",
                "DE", LeadStatus.ACTIVE, LocalDateTime.now()
            ).list();
            dbTimes.add(TimeUnit.NANOSECONDS.toMillis(System.nanoTime() - startTime));
        }

        long p95 = calculateP95(dbTimes);
        assertTrue(p95 < DB_QUERY_THRESHOLD_MS,
    ...
    void testConcurrentAccessPerformance() throws Exception {
        ...
        List<Long> concurrentTimes = futures.stream()
            .map(f -> {
                try {
        ...
        assertTrue(p95 < P95_THRESHOLD_MS * 1.5, // Allow 50% degradation under load
    ...
    void testETagPerformance() {
        ...
        if (etag != null) {
            ...
            long avgEtagTime = etagTimes.stream()
                .mapToLong(Long::longValue)
                .sum() / etagTimes.size();

            assertTrue(avgEtagTime < 50,
    ...
    void testAggregationPerformance() {
        ...
        assertTrue(p95 < DB_QUERY_THRESHOLD_MS * 2, // Allow 2x for aggregations
