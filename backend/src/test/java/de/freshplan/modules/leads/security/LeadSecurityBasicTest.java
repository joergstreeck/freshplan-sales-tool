 ...
 *
 * Simplified version that actually compiles and runs.
 * These tests validate basic security concepts without
...
public class LeadSecurityBasicTest {
    ...
    void testTerritoryIsolation() {
        ...
        assertNotEquals(leadDE.territory.id, leadCH.territory.id,
            "Leads should have different territories");
        assertEquals("EUR", leadDE.territory.currencyCode,
            "DE lead should use EUR");
        assertEquals("CHF", leadCH.territory.currencyCode,
            "CH lead should use CHF");
    }
    ...
    void testOwnerIdentification() {
        ...
        assertEquals(ownerId, lead.ownerUserId,
        ...
        assertEquals(2, lead.collaboratorUserIds.size(),
