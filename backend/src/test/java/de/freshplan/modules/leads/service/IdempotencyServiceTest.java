...
class IdempotencyServiceTest {
    ...
    void shouldStoreAndRetrieveIdempotencyKey() {
        ...
        // When - Store
        boolean stored = idempotencyService.storeIdempotencyKey(
            tenantId, idempotencyKey, requestBody, responseBody
        );
        ...
        Optional<String> retrieved = idempotencyService.checkIdempotencyKey(
    ...
    void shouldRejectDifferentRequestWithSameIdempotencyKey() {
        ...
        idempotencyService.storeIdempotencyKey(
            tenantId, idempotencyKey, originalRequest, responseBody
        );

        // Then - Storing with different request should return false
        boolean result = idempotencyService.storeIdempotencyKey(
    ...
    void shouldReturnCachedResponseForSameRequest() {
        ...
        Optional<String> cached = idempotencyService.checkIdempotencyKey(
            tenantId, idempotencyKey
        );
    ...
    void shouldHandleNullRequestBody() {
        ...
        boolean stored = idempotencyService.storeIdempotencyKey(
            tenantId, idempotencyKey, null, responseBody
        );
    ...
    void shouldReturnEmptyWhenNoIdempotencyKeyProvided() {
        // When
        Optional<String> result = idempotencyService.checkIdempotencyKey(
            "tenant-1", null
        );
    ...
    void shouldIsolateTenants() {
        ...
        idempotencyService.storeIdempotencyKey(
            "tenant-1", idempotencyKey, requestBody, responseBody1
        );
        idempotencyService.storeIdempotencyKey(
            "tenant-2", idempotencyKey, requestBody, responseBody2
        );

        // Then - Each tenant gets their own response
        Optional<String> result1 = idempotencyService.checkIdempotencyKey(
            "tenant-1", idempotencyKey
        );
        Optional<String> result2 = idempotencyService.checkIdempotencyKey(
            "tenant-2", idempotencyKey
        );
