 ...
 *
 * Sprint 2.1 - FP-235: Follow-up Automation Tests
 */
@QuarkusTest
class FollowUpAutomationServiceTest {
    ...
    void testT3FollowUpProcessing() {
        ...
        // Mock email service
        when(emailService.sendCampaignEmail(any(Lead.class), any(CampaignTemplate.class), any(Map.class)))
            .thenReturn(true);
        ...
        ArgumentCaptor<CampaignTemplate> templateCaptor = ArgumentCaptor.forClass(CampaignTemplate.class);
        ArgumentCaptor<Map> dataCaptor = ArgumentCaptor.forClass(Map.class);

        verify(emailService, atLeastOnce()).sendCampaignEmail(
            leadCaptor.capture(),
        ...
        verify(followUpEvent).fire(any(FollowUpProcessedEvent.class));
    }
    ...
    void testNoFollowUpForRecentActivity() {
        ...
        verify(emailService, never()).sendCampaignEmail(
            any(Lead.class),
            any(CampaignTemplate.class),
            any(Map.class)
        );
    ...
    void testSeasonalSampleRecommendations() {
        ...
        // Mock email service
        ArgumentCaptor<Map> dataCaptor = ArgumentCaptor.forClass(Map.class);
        when(emailService.sendCampaignEmail(any(Lead.class), any(CampaignTemplate.class), dataCaptor.capture()))
            .thenReturn(true);
    ...
    @Test
    void testFollowUpEventContent() {
        // Given: Multiple leads for T+3 and T+7
        // Mock the results
        when(emailService.sendCampaignEmail(any(Lead.class), any(CampaignTemplate.class), any(Map.class)))
            .thenReturn(true);

        ArgumentCaptor<FollowUpProcessedEvent> eventCaptor = ArgumentCaptor.forClass(FollowUpProcessedEvent.class);

        // When: Processing happens
        followUpService.processScheduledFollowUps();

        // Then: Event contains correct counts
