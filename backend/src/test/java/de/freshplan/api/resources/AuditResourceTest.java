...
class AuditResourceTest {
    ...
    @Test
    @TestSecurity(user = "user", roles = {"sales"})
    ...
    void testSearchAuditTrail() {
        // Given
        List<AuditEntry> mockEntries = List.of(createMockAuditEntry(AuditEventType.OPPORTUNITY_CREATED));
    ...
    void testGetStatistics() {
        // Given
        var mockStats = AuditRepository.AuditStatistics.builder()
                .totalEvents(100L)
                .uniqueUsers(10L)
                .uniqueEntities(50L)
                .failureCount(5L)
    ...
    @Test
    @TestSecurity(user = "admin", roles = {"admin", "security"})
    ...
    void testVerifyIntegrity_Compromised() {
        // Given
        var issue = new AuditRepository.AuditIntegrityIssue(
            UUID.randomUUID(),
            "Hash chain broken",
            "expected-hash",
            "actual-hash"
    ...
    void testPaginationParameters() {
        // Given
        when(auditRepository.findByEntity(any(), any(), eq(2), eq(25)))
                .thenReturn(List.of());
