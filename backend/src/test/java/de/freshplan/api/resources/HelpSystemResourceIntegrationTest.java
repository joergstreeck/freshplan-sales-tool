 ...
 *
 * <p>Verifiziert, dass alle REST Endpoints korrekt mit dem neuen Event-Driven CQRS System
 * funktionieren.
 *
 * <p>Testet: - Alle 8 Endpoints der HelpSystemResource - Korrekte Integration mit CQRS Services -
...
class HelpSystemResourceIntegrationTest {
  ...
  void submitFeedback_withValidData_shouldRecordFeedback() {
    Map<String, Object> feedbackRequest =
        Map.of(
            "helpId",
            testHelpContentId.toString(),
            "userId",
            "test-user",
            "helpful",
            true,
  ...
  void submitFeedback_withInvalidHelpId_shouldReturnBadRequest() {
    Map<String, Object> feedbackRequest =
        Map.of("helpId", UUID.randomUUID().toString(), "userId", "test-user", "helpful", false);
  ...
  void toggleHelpContent_activate_shouldSucceed() {
    // First deactivate
    given().queryParam("active", false).put("/api/help/content/" + testHelpContentId + "/toggle");
