...
class ContactInteractionResourceIT {
    ...
    void shouldGetDataQualityMetricsEmpty() {
        given()
            ...
            .then()
                ...
                .body("overallDataQuality", oneOf("CRITICAL", "POOR", "FAIR", "GOOD", "EXCELLENT", "UNKNOWN"));
    }
    ...
    void shouldRecordCall() {
        given()
            .contentType(ContentType.JSON)
            .body("{\"type\": \"OUTBOUND\", \"duration\": 1800, \"outcome\": \"Successful discussion\"}")
            .when()
    ...
    void shouldImproveDataQualityAfterInteractions() {
        // Get initial metrics
        DataQualityMetricsDTO initialMetrics = given()
            .when()
                .get("/api/contact-interactions/metrics/data-quality")
            .then()
                .statusCode(200)
