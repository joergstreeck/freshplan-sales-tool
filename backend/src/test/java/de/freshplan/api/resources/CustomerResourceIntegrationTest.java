 ...
 * 
 * Testet alle 13 Endpoints mit echten HTTP-Requests und PostgreSQL-Datenbank.
 * Nutzt RestAssured für API-Tests und Testcontainers für die DB.
 * 
...
public class CustomerResourceIntegrationTest extends BaseIntegrationTest {
    ...
    void restoreCustomer_shouldReturn200() {
        ...
        // Now restore
        CustomerResponse restored = given()
            .when()
    ...
    void getAllCustomers_withPagination_shouldReturn200() {
        ...
        CustomerListResponse response = given()
            .queryParam("page", 0)
            .queryParam("size", 3)
            .when()
            .get()
        ...
        assertTrue(response.totalElements() >= 5, "Expected at least 5 elements, got " + response.totalElements());
        // There should be at least 2 pages
        assertTrue(response.totalPages() >= 2, "Expected at least 2 pages, got " + response.totalPages());
    ...
    void getAllCustomers_withStatusFilter_shouldReturnFiltered() {
        ...
        assertTrue(response.content().stream()
            .allMatch(c -> c.status() == CustomerStatus.AKTIV), 
    ...
    void getDashboardData_shouldReturnMetrics() {
        // Get baseline count
        CustomerDashboardResponse baseline = given()
            .when()
            .get("/dashboard")
            .then()
            .statusCode(200)
        ...
        CustomerDashboardResponse response = given()
            .when()
            .get("/dashboard")
            .then()
            .statusCode(200)
        ...
        assertTrue(response.totalCustomers() >= baselineCount + 3, 
            "Expected at least " + (baselineCount + 3) + " customers, got " + response.totalCustomers());
    ...
    void getCustomerHierarchy_shouldReturnHierarchyTree() {
        ...
        CustomerResponse hierarchy = given()
            .when()
    ...
    void addChildCustomer_shouldLinkCustomers() {
        ...
        CustomerResponse updatedChild = given()
            .contentType(ContentType.JSON)
            .body(request)
            .when()
    ...
    void checkDuplicates_shouldReturnPotentialDuplicates() {
        ...
        List<CustomerResponse> duplicates = given()
            .contentType(ContentType.JSON)
            .body(request)
            .when()
        ...
        assertTrue(duplicates.stream()
            .allMatch(c -> c.companyName().contains("ABC Company")));
    }
    ...
    void changeCustomerStatus_shouldUpdateStatus() {
        ...
        CustomerResponse updated = given()
            .contentType(ContentType.JSON)
            .body(request)
            .when()
