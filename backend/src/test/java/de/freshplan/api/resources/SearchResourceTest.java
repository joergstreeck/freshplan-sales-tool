 ...
 * 
 * Tests cover:
 * - Universal search endpoint with various query types
 * - Quick search endpoint for autocomplete
 * - Security role validation
 * - Input validation and error handling
 * - Response format verification
 * 
 * Applied Test Patterns:
 * - TestSecurity for role-based access
 * - @InjectMock for service layer isolation
...
class SearchResourceTest {
    ...
    @Test
    @TestSecurity(user = "testuser", roles = {"unauthorized"})
    ...
    void quickSearch_withValidParams_shouldReturnQuickResults() {
        ...
        when(searchService.quickSearch(eq(query), eq(5)))
            .thenReturn(mockResults);
    ...
    void quickSearch_withDefaultLimit_shouldUseDefault() {
        // Given
        SearchResults mockResults = createMockQuickSearchResults();
        when(searchService.quickSearch(eq("test"), eq(5)))
            .thenReturn(mockResults);
    ...
    private SearchResults createMockSearchResults() {
        ...
        SearchResult customerResult = SearchResult.builder()
            .type("customer")
            .id("customer-1")
            .data(customerDto)
            .relevanceScore(85)
        ...
        SearchResult contactResult = SearchResult.builder()
            .type("contact")
            .id("contact-1")
            .data(contactDto)
            .relevanceScore(90)
