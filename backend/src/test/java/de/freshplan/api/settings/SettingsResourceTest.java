...
/**
 * Tests for Settings REST API (Sprint 1.2 PR #2).
 * Verifies ETag headers, conditional requests, and optimistic locking.
 */
@QuarkusTest
...
class SettingsResourceTest {
    ...
    void testCreateSetting() {
        ...
        JsonObject metadata = new JsonObject()
            .put("description", "Test API setting");
    ...
    void testUpdateWithOptimisticLocking() {
        ...
        Setting setting = settingsService.saveSetting(
            SettingsScope.GLOBAL, null, TEST_KEY,
            initialValue, null, "test-user"
        );
    ...
    void testHierarchicalResolution() {
        ...
        settingsService.saveSetting(
            SettingsScope.GLOBAL, null, "hierarchical.test",
            new JsonObject().put("level", "global"), null, "test-user"
        );

        settingsService.saveSetting(
            SettingsScope.TENANT, "freshfoodz", "hierarchical.test",
            new JsonObject().put("level", "tenant"), null, "test-user"
        );
        ...
        .when()
            .get("/api/settings/resolve/{key}", "hierarchical.test")
        .then()
            .statusCode(200)
    ...
    void testListSettings() {
        ...
        for (int i = 1; i <= 3; i++) {
            settingsService.saveSetting(
                SettingsScope.TENANT, "freshfoodz", "list.test" + i,
                new JsonObject().put("index", i), null, "test-user"
            );
        }
