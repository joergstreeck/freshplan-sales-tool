 ...
 * 
 * Named AAA_ to ensure alphabetical priority over all other tests.
 * This test identifies which tests are creating customers without proper test data markers.
 */
@QuarkusTest
public class AAA_EarlyBirdDebugTest {
    ...
    void aaa_captureInitialDatabasePollution() {
        ...
        List<Customer> problematicCustomers = customerRepository
        ...
        if (!problematicCustomers.isEmpty()) {
            ...
            problematicCustomers.forEach(c -> {
        ...
        for (String pattern : problematicPatterns) {
            ...
            if (count > 0) {
                ...
                // Show details
                customerRepository.find("companyName LIKE ?1", pattern)
                    .page(0, 3)
                    .list()
                    .forEach(c -> {
        ...
        System.out.println("\n### TIMELINE CHECK ###");
        if (totalCustomers > 0) {
            Customer oldest = customerRepository
                .find("ORDER BY createdAt ASC")
                .firstResult();
            if (oldest != null) {
                System.out.printf("Oldest customer: %s (created at %s)%n", 
                    oldest.getCompanyName(), oldest.getCreatedAt());
            }
            
            Customer newest = customerRepository
                .find("ORDER BY createdAt DESC")
                .firstResult();
            if (newest != null) {
                System.out.printf("Newest customer: %s (created at %s)%n", 
    ...
    private void identifyPotentialSource(Customer c) {
        ...
        if (name.contains("Parent Company") || name.contains("Child Company")) {
            System.out.println("    ⚠️  LIKELY SOURCE: CustomerCQRSIntegrationTest.addChildCustomer_inCQRSMode_shouldCreateHierarchy()");
        } else if (name.contains("Source Company") || name.contains("Target Company")) {
            System.out.println("    ⚠️  LIKELY SOURCE: CustomerCQRSIntegrationTest.mergeCustomers_inCQRSMode_shouldMergeData()");
            ...
            System.out.println("    ⚠️  LIKELY SOURCE: OpportunityServiceStageTransitionTest or OpportunityResourceIntegrationTest");
        }
    }
}