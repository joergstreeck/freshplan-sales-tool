 ...
 *
 * <p>Named ZZZ_ to ensure it runs after all other tests alphabetically. This test verifies that all
...
@Tag("quarantine")public class ZZZ_FinalVerificationTest {
  ...
  void zzz_verifyAllTestDataProperlyMarked() {
    ...
    if (!stillProblematic.isEmpty()) {
      ...
      for (Customer c : stillProblematic) {
        ...
        if (name.contains("Test Company") && !name.contains("[TEST]")) {
          System.out.println(
              "⚠️  OpportunityServiceStageTransitionTest or OpportunityResourceIntegrationTest");
    ...
    if (!stillProblematic.isEmpty()) {
      ...
      // throw new AssertionError("Found " + stillProblematic.size() + " customers without proper
      // test markers!");
    }
  }
}
