 ...
 * 
 * <p>Implements the team's recommendations:
 * - Explicit test data setup per test
 * - FK-safe cleanup in correct order
 * - Dynamic IDs for unique fields
...
public abstract class BaseIntegrationTestWithCleanup {
   ...
   * Performs FK-safe cleanup in correct order.
   */
  @AfterEach
  @Transactional
  ...
  private void cleanupContacts() {
    ...
    long deleted = contactRepository.delete(
  ...
  private void cleanupOpportunities() {
    long deleted = opportunityRepository.delete(
  ...
  /**
   * Helper to create a test customer with all required fields.
   * Uses TestDataBuilder for dynamic unique values.
   */
  ...
  protected de.freshplan.domain.user.entity.User getOrCreateTestUser(String username) {
    return userRepository.findByUsername(username)
        .orElseGet(() -> {
          var user = new de.freshplan.domain.user.entity.User(
              username, 
              "Test", 
              "User", 
              username + "@test.de"
