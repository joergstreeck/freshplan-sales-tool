 ...
 * 
 * GARANTIEN:
 * 1. Jeder Test startet mit exakt 50 Seed-Kunden (SEED-001 bis SEED-050)
 * 2. Alle Änderungen werden nach dem Test zurückgerollt
 * 3. Keine gegenseitige Beeinflussung zwischen Tests
 * 4. Stabile, vorhersagbare Test-Umgebung
 * 
 * VERWENDUNG:
 * - Alle Integration-Tests MÜSSEN von dieser Klasse erben
 * - Verwende @TestTransaction für automatisches Rollback
 * - Seed-Daten sind immer verfügbar und unverändert
 * 
 * @author Claude
...
public abstract class IsolatedIntegrationTest {
     ...
     * Verifiziert dass Seed-Daten vorhanden sind.
     */
    @BeforeEach
    protected void verifyTestEnvironment() {
        // Verifiziere Seed-Daten
        Long seedCount = em.createQuery(
            "SELECT COUNT(c) FROM Customer c WHERE c.customerNumber LIKE 'SEED-%'", 
            Long.class
        ).getSingleResult();
        
        if (seedCount != SEED_CUSTOMER_COUNT) {
            LOG.warnf("Erwartete %d Seed-Kunden, gefunden: %d", 
                     SEED_CUSTOMER_COUNT, seedCount);
        }
     ...
     * Entfernt NUR Test-Daten, NIEMALS Seed-Daten.
     */
    @AfterEach
    ...
    /**
     * Entfernt alte Test-Daten von vorherigen Test-Läufen.
     * Wird nur ausgeführt wenn Tests nicht korrekt beendet wurden.
     */
    ...
    protected long countNonSeedCustomers() {
        return em.createQuery(
            "SELECT COUNT(c) FROM Customer c WHERE c.customerNumber NOT LIKE 'SEED-%'",
            Long.class
        ).getSingleResult();
    }
