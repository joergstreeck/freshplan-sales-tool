 ...
 * 
 * This class ensures that test data created during integration tests
 * is properly cleaned up to prevent database pollution.
 * 
 * @author Claude
...
public class TestDataCleanup {
    ...
    /**
     * Clean up all test customers with the given test run ID.
     * This method should be called in @AfterEach of integration tests.
     * 
    ...
    public void cleanupTestRun(String testRunId) {
        ...
        QuarkusTransaction.requiringNew().run(() -> {
            try {
                ...
                long deletedCustomers = customerRepository.delete(
                ...
                if (deletedCustomers > 0) {
                    LOG.infof("Cleaned up %d test customers for run: %s", deletedCustomers, testRunId);
                }
                
                // Also clean up any customers with specific prefixes that might contain the testRunId
                String[] prefixes = {"QK", "PF", "PG", "E1", "E2", "S1", "S2", "PA", "PI", "EX", "PT", "ACT", "INA"};
                for (String prefix : prefixes) {
                    long deleted = customerRepository.delete(
                        "customerNumber LIKE ?1",
                        prefix + "%" + testRunId.substring(0, Math.min(testRunId.length(), 5)) + "%"
                    );
                    if (deleted > 0) {
     ...
     * This is a safety net for tests that fail to clean up properly.
     */
