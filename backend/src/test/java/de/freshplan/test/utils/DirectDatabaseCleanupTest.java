...
public class DirectDatabaseCleanupTest {
    ...
    void performDirectCleanup() {
        ...
        QuarkusTransaction.requiringNew().run(() -> {
            ...
            List<Customer> suspiciousCustomers = customerRepository.find(
            ...
            // Show a sample
            suspiciousCustomers.stream().limit(10).forEach(c -> 
                LOG.infof("  - %s: %s", c.getCustomerNumber(), c.getCompanyName())
            );
        });
        ...
        QuarkusTransaction.requiringNew().run(() -> {
            ...
            String[] patterns = {"PF%", "PA%", "PI%", "QK%", "PG%", "E1%", "E2%", 
                                "S1%", "S2%", "EX%", "PT%", "ACT%", "INA%"};
            ...
            long deletedSpecific = customerRepository.delete(
                "companyName LIKE ?1 OR companyName LIKE ?2 OR companyName LIKE ?3 " +
                "OR companyName LIKE ?4 OR companyName LIKE ?5",
                "%Quick Test%", "%Performance Test%", "%Pagination Test%", 
                "%Priority%", "%Relevance Test%"
            );
        ...
        QuarkusTransaction.requiringNew().run(() -> {
            ...
            if (remaining > 100) {
                ...
                remainingCustomers.stream()
                    ...
                    .forEach(c -> {
                        LOG.warnf("Suspicious customer still present: %s - %s", 
            ...
            for (Customer c : all) {
                ...
                if (c.getCompanyName() != null) {
                    ...
                    if (name.contains("[TEST") || name.contains("Test ") || 
                        name.contains("Performance") || name.contains("Quick") ||
                        name.contains("Pagination") || name.contains("Priority") ||
