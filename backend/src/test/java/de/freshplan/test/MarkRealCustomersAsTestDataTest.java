...
/**
 * Markiert die verbleibenden 41 "echten" Kunden als Test-Daten,
 * damit sie eindeutig von produktiven Daten unterscheidbar sind.
 * 
 * Diese Kunden sind keine produktiven Daten, sondern Teil der Test-Umgebung.
 */
@QuarkusTest
public class MarkRealCustomersAsTestDataTest {
    ...
    public void markRemainingCustomersAsTestData() {
        ...
        List<Customer> unmarkedCustomers = customerRepository.list(
            "isTestData = ?1 OR isTestData IS NULL", false
        );
        ...
        for (Customer customer : unmarkedCustomers) {
            ...
            if (updated <= 10) {
                System.out.println(String.format("  Markiert: %s - %s", 
        ...
        long testDataCount = allCustomers.stream()
            .filter(c -> Boolean.TRUE.equals(c.getIsTestData()))
            .count();
        ...
        customerRepository.find("isTestData = ?1", true)
            .range(0, 5)
            .list()
            .forEach(c -> System.out.println(String.format("  - %s: %s (is_test_data=%s)", 
                c.getCustomerNumber(), 
                c.getCompanyName(),
                c.getIsTestData())));
    ...
    public void verifyAllCustomersAreTestData() {
        ...
        List<Customer> nonTestCustomers = customerRepository.list(
            "isTestData = ?1 OR isTestData IS NULL", false
        );
        ...
        if (!nonTestCustomers.isEmpty()) {
            ...
            nonTestCustomers.forEach(c -> System.out.println(String.format("  - %s: %s (is_test_data=%s)", 
                c.getCustomerNumber(), 
                c.getCompanyName(),
                c.getIsTestData())));
