...
public class TestDataIntegrityCheckTest {
  ...
  void findInconsistentTestData() {
    ...
    List<Customer> suspiciousCustomers =
        customerRepository
            .find(
                "(companyName like ?1 or companyName like ?2 or customerNumber like ?3) and (isTestData = false or isTestData is null)",
                "%Test%",
                "%test%",
                "KD%")
            .list();
    ...
    List<Customer> inconsistentTestPrefix =
        customerRepository
    ...
    if (!inconsistentTestPrefix.isEmpty()) {
      System.out.println(
          "Found "
              + inconsistentTestPrefix.size()
              + " customers with [TEST] prefix but wrong flag:");
      inconsistentTestPrefix.forEach(
          c -> {
    ...
    List<Customer> testUserCreated =
        customerRepository
            .find(
                "createdBy in (?1)",
                List.of("testuser", "test-user", "test-data-seeder", "testuser1", "testuser2"))
            .list();
    ...
    if (testUserCreated.size() > markedAsTest) {
      System.out.println(
          "⚠️ WARNING: More customers created by test users than marked as test data!");
    }
