...
class CiFkSanityIT {
  ...
  private static String actionName(char del) {
    switch (del) {
      ...
      default: return String.valueOf(del);
    }
  }
   ...
   * 
   * Now uses OID-based self-reference detection for robustness.
   */
  @Test
  @Order(1)
  void fkActionsForCustomersAreAsExpected() throws Exception {
    ...
    try (var c = ds.getConnection();
      ...
      while (rs.next()) {
        ...
        System.out.printf("FK child=%s parent=%s self=%s action=%s%n",
            child, parent, self, actionName(del));
        ...
        if (self) {
          ...
          if (del == 'n') {
            ...
            System.out.printf("WARNING: Self-FK action should be SET NULL, but is %s%n", 
                actionName(del));
            unexpected++;
          }
        } else {
          ...
          if (del == 'c') {
            ...
            System.out.printf("WARNING: Non-self FK action should be CASCADE, but is %s (child=%s)%n",
    ...
    assertEquals(0, unexpected, 
      String.format("Found %d FKs with unexpected delete actions", unexpected));
  }
}