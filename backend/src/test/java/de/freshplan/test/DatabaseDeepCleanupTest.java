...
public class DatabaseDeepCleanupTest {
    ...
    public void performDeepCleanup() {
        ...
        int deletedInteractions = em.createQuery(
        ...
        System.out.println("   Deleted " + deletedInteractions + " of " + interactions + " interactions");
        
        // Schritt 2: Lösche alle Contacts
        System.out.println("\n2. Cleaning up Customer Contacts...");
        long contacts = contactRepository.count();
        int deletedContacts = em.createQuery(
        ...
        int deletedOpportunities = em.createQuery(
        ...
        System.out.println("   Deleted " + deletedOpportunities + " of " + opportunities + " opportunities");
        
        // Schritt 4: Jetzt können wir Customers löschen
        System.out.println("\n4. Cleaning up Test Customers...");
        
        // Pattern-basiertes Löschen
        int deleted1 = em.createQuery("DELETE FROM Customer c WHERE c.customerNumber LIKE 'KD-S%'").executeUpdate();
        System.out.println("   Deleted " + deleted1 + " KD-S* customers");
        
        int deleted2 = em.createQuery("DELETE FROM Customer c WHERE c.customerNumber LIKE 'KD-E%'").executeUpdate();
        System.out.println("   Deleted " + deleted2 + " KD-E* customers");
        
        int deleted3 = em.createQuery("DELETE FROM Customer c WHERE c.customerNumber LIKE 'KD-EVT%'").executeUpdate();
        System.out.println("   Deleted " + deleted3 + " KD-EVT* customers");
        
        // Name-basiertes Löschen
        int deleted4 = em.createQuery("DELETE FROM Customer c WHERE c.companyName LIKE '%Test%'").executeUpdate();
        System.out.println("   Deleted " + deleted4 + " customers with 'Test' in name");
        
        int deleted5 = em.createQuery("DELETE FROM Customer c WHERE c.companyName LIKE '%Search Solutions%'").executeUpdate();
        System.out.println("   Deleted " + deleted5 + " 'Search Solutions' customers");
        
        int deleted6 = em.createQuery("DELETE FROM Customer c WHERE c.companyName LIKE '%Export%'").executeUpdate();
    ...
    public void analyzeOnly() {
        ...
        System.out.println("  KD-EVT*: " + customerRepository.count("customerNumber LIKE ?1", "KD-EVT%"));
