...
/**
 * SEHR DETAILLIERTE Analyse der Datenbank-Situation
 * Bevor wir irgendetwas löschen, verstehen wir GENAU was los ist!
 */
@QuarkusTest
public class DetailedDatabaseAnalysisTest {
    ...
    public void performDetailedAnalysis() {
        ...
        System.out.println("Analyse-Zeitpunkt: " + LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME));
    ...
    private void analyzeCustomerPatterns() {
        ...
        patterns.put("KD-PG* (Pagination)", customerRepository.count("customerNumber LIKE ?1", "KD-PG%"));
        patterns.put("KD-E1*/KD-E2*", customerRepository.count("customerNumber LIKE ?1 OR customerNumber LIKE ?2", "KD-E1%", "KD-E2%"));
        patterns.put("Search Solutions", customerRepository.count("companyName LIKE ?1", "%Search Solutions%"));
        patterns.put("Export Hotel/Restaurant", customerRepository.count("companyName LIKE ?1 OR companyName LIKE ?2", "%Export Hotel%", "%Export Restaurant%"));
        patterns.put("Priority Customer", customerRepository.count("companyName LIKE ?1", "%Priority Customer%"));
        patterns.put("Relevance Test", customerRepository.count("companyName LIKE ?1", "%Relevance Test%"));
    ...
    private void analyzeTemporalDistribution() {
        ...
        Query query = em.createQuery(
            "SELECT COUNT(c) FROM Customer c WHERE c.createdAt > :time"
        );
        ...
        Query oldestQuery = em.createQuery(
            "SELECT c FROM Customer c ORDER BY c.createdAt ASC", Customer.class
        ).setMaxResults(1);
        
        Query newestQuery = em.createQuery(
        ...
        if (!oldest.isEmpty()) {
            ...
            System.out.println("\nÄltester Kunde: " + c.getCustomerNumber() + " - " + c.getCompanyName() + 
                " (erstellt: " + c.getCreatedAt() + ")");
        }
        
        if (!newest.isEmpty()) {
            Customer c = newest.get(0);
            System.out.println("Neuester Kunde: " + c.getCustomerNumber() + " - " + c.getCompanyName() + 
                " (erstellt: " + c.getCreatedAt() + ")");
        }
    }
    ...
    private void analyzeForeignKeyDependencies() {
        ...
        Query customersWithContactsQuery = em.createQuery(
            "SELECT COUNT(DISTINCT c.customer.id) FROM CustomerContact c"
        );
        System.out.println("Kunden mit Kontakten: " + customersWithContactsQuery.getSingleResult());
        
        Query customersWithOpportunitiesQuery = em.createQuery(
            "SELECT COUNT(DISTINCT o.customer.id) FROM Opportunity o"
        );
        System.out.println("Kunden mit Opportunities: " + customersWithOpportunitiesQuery.getSingleResult());
        
        // Kunden mit Interactions (indirekt über Contacts)
        Query customersWithInteractionsQuery = em.createQuery(
            "SELECT COUNT(DISTINCT cc.customer.id) FROM ContactInteraction ci " +
            "JOIN ci.contact cc"
        );
        System.out.println("Kunden mit Interactions (indirekt): " + customersWithInteractionsQuery.getSingleResult());
        
        // Test-Kunden mit Dependencies
        Query testCustomersWithDeps = em.createQuery(
        ...
        Query testCustomersWithoutDeps = em.createQuery(
        ...
        System.out.println("Test-Kunden OHNE Dependencies (löschbar): " + testCustomersWithoutDeps.getSingleResult());
    }
    
    private void analyzeDuplicates() {
        // Gibt es mehrfache Test-Kunden mit ähnlichen Namen?
        Query duplicateQuery = em.createQuery(
        ...
        Query similarQuery = em.createQuery(
    ...
    private void distinguishOriginalFromTestData() {
        ...
        Query originalTestData = em.createQuery(
        ...
        if (originals.size() <= 10) {
            for (Customer c : originals) {
                System.out.printf("  - %s: %s (erstellt: %s)\n", 
        ...
        long nonTestCustomers = customerRepository.count(
    ...
    private void findProblematicCustomers() {
        ...
        Query problematicQuery = em.createQuery(
        ...
        for (Object[] row : problematic) {
            System.out.printf("  %s - %s: %d Kontakte, %d Opportunities\n",
