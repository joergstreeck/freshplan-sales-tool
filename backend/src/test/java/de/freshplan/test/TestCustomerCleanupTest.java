 ...
 * 
 * WICHTIG: Nur Test-Kunden löschen, Original-Daten behalten!
 */
@QuarkusTest
public class TestCustomerCleanupTest {
    ...
    public void cleanupTestCustomersToOriginal74() {
        ...
        List<Customer> originalCustomers = customerRepository.list(
            "companyName LIKE ?1", "[TEST]%"
        );
        ...
        int deletedTimelineEvents = em.createQuery(
        ...
        int deleted1 = em.createQuery(
            "DELETE FROM Customer c WHERE c.customerNumber LIKE 'KD-S%'"
        ).executeUpdate();
        System.out.println("  KD-S* gelöscht: " + deleted1);
        
        int deleted2 = em.createQuery(
            "DELETE FROM Customer c WHERE c.customerNumber LIKE 'KD-E%'"
        ).executeUpdate();
        System.out.println("  KD-E* gelöscht: " + deleted2);
        
        int deleted3 = em.createQuery(
            "DELETE FROM Customer c WHERE c.customerNumber LIKE 'KD-EVT%'"
        ).executeUpdate();
        System.out.println("  KD-EVT* gelöscht: " + deleted3);
        
        int deleted4 = em.createQuery(
            "DELETE FROM Customer c WHERE c.customerNumber LIKE 'KD-PG%'"
        ).executeUpdate();
        System.out.println("  KD-PG* gelöscht: " + deleted4);
        
        int deleted5 = em.createQuery(
            "DELETE FROM Customer c WHERE c.customerNumber LIKE 'KD-CQRS%'"
        ).executeUpdate();
        System.out.println("  KD-CQRS* gelöscht: " + deleted5);
        
        // Lösche Test-Kunden mit "Test" im Namen (aber NICHT [TEST] Prefix!)
        int deleted6 = em.createQuery(
