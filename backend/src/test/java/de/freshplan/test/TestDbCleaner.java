 ...
 *
 * <p>Deletes child records before parent records to avoid foreign key violations. Used in
 * integration tests to ensure clean state between tests.
 */
@ApplicationScoped
public class TestDbCleaner {
  ...
  public void cleanAllTestData() {
    try (Connection conn = dataSource.getConnection()) {
      ...
      try (PreparedStatement ps =
          conn.prepareStatement("DELETE FROM customers WHERE is_test_data = true")) {
        int deleted = ps.executeUpdate();
  ...
  public void cleanDataByUser(String testUsername) {
    try (Connection conn = dataSource.getConnection()) {
      ...
      String[] tables = {
        "customer_timeline_events", "customer_contacts", "opportunities", "customers"
      };

      for (String table : tables) {
        try (PreparedStatement ps =
            conn.prepareStatement("DELETE FROM " + table + " WHERE created_by = ?")) {
  ...
  private void deleteFromTable(Connection conn, String table, String column, UUID value)
      throws SQLException {
