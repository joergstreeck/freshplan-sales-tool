...
/**
 * Verifies that SEED data from V10005 migration is present and correct. This test should run early
 * to detect the Two-Container Problem.
 */
@QuarkusTest
@Tag("quarantine")
class SeedDataVerificationTest {
  ...
  void verify_seed_data_present_and_protected() {
    ...
    try (Connection conn = dataSource.getConnection();
      ...
      while (rs.next()) {
        LOG.infof("  V%s: %s (success=%s)", rs.getString(1), rs.getString(2), rs.getBoolean(3));
      }

      // Count SEED customers directly
      ResultSet rs2 =
          stmt.executeQuery("SELECT COUNT(*) FROM customers WHERE customer_number LIKE 'SEED-%'");
      rs2.next();
      ...
      if (seedCountRaw > 0) {
        ...
        while (rs3.next()) {
          LOG.infof(
              "  %s: %s (is_test_data=%s)", rs3.getString(1), rs3.getString(2), rs3.getBoolean(3));
        }
      }
    ...
    assertThat(seedCustomers).as("Should have exactly 20 SEED customers from V10005").hasSize(20);
