...
-- NOTE: Removed CONCURRENTLY for CI compatibility (can't run in transaction)
CREATE INDEX IF NOT EXISTS idx_customers_active_company_name
...
-- Note: In production, recreate these indexes with CONCURRENTLY:
-- DROP INDEX idx_customers_active_company_name;
-- CREATE INDEX CONCURRENTLY idx_customers_active_company_name ON customers(is_deleted, company_name) WHERE is_deleted = false;
-- etc.
