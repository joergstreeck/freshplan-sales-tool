-- V8: Add Data Quality and Freshness Tracking fields to customer_contacts table
...
CREATE INDEX IF NOT EXISTS idx_contacts_data_quality_score ON customer_contacts(data_quality_score);

-- Create index for freshness queries (using existing updated_at field)
CREATE INDEX IF NOT EXISTS idx_contacts_updated_at ON customer_contacts(updated_at);

-- Create standard indices for better query performance
CREATE INDEX IF NOT EXISTS idx_contacts_data_quality_low ON customer_contacts(data_quality_score);
