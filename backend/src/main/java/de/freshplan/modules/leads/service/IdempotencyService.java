...
/**
 * Service to handle idempotent API operations.
 * Sprint 2.1.4: Lead Deduplication & Data Quality
 *
 * Prevents duplicate processing of API requests by storing and checking
 * idempotency keys with a TTL (Time To Live).
 */
@ApplicationScoped
public class IdempotencyService {
    ...
    public Optional<String> checkIdempotencyKey(String tenantId, String idempotencyKey) {
        ...
        try {
            var result = entityManager.createNativeQuery(
                ...
                "AND expires_at > :now", String.class)
     ...
     */
    @Transactional
    public boolean storeIdempotencyKey(String tenantId, String idempotencyKey,
    ...
    @Transactional
    public boolean storeIdempotencyKey(String tenantId, String idempotencyKey,
                                       String requestBody, String responseBody, int responseStatus) {
        if (idempotencyKey == null || idempotencyKey.isBlank()) {
        ...
        if (inserted == 0) {
            ...
            try {
                ...
                if (!existingHash.equals(requestHash)) {
                    Log.warnf("Idempotency key reused with different request. " +
                             "Tenant: %s, Key: %s", tenantId, idempotencyKey);
                    return false;
                }
                // Same request, already processed
                Log.debugf("Idempotency key already exists for same request. Tenant: %s, Key: %s",
