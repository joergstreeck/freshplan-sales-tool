 ...
 *
 * Sprint 2.1: Mock-Implementation für Follow-up Automation
 * Sprint 3.x: Integration mit SendGrid/AWS SES
 */
@ApplicationScoped
public class EmailProviderService {
     ...
     * Loggt Emails statt sie zu senden
     */
    private boolean sendMocked(EmailNotificationService.EmailMessage message) {
        if (logEmails) {
            LOG.infof("""
                ...
                message.getToName(), message.getTo(),
                message.getSubject(),
                message.getLeadId(),
                message.getTemplateId(),
                message.getHtmlContent() != null ?
                    message.getHtmlContent().substring(0,
                        Math.min(500, message.getHtmlContent().length())) : "N/A"
            );
        }
