 ...
 *
 * Stellt sicher, dass in Production ein sicheres Secret konfiguriert ist
 * und verhindert den Start mit Development-Fallback
 */
@Startup
@ApplicationScoped
    ...
    void verifySecretConfiguration() {
        ...
        if (isProduction) {
            ...
            if (secret.isEmpty() || secret.equals(DEV_SECRET)) {
                LOG.warn("⚠️ Using development token secret. Configure freshplan.unsubscribe.token.secret for production!");
            }
        }
    }

    /**
     * Liefert das konfigurierte Secret oder Development-Fallback
     * Wird von EmailNotificationService verwendet
     */
