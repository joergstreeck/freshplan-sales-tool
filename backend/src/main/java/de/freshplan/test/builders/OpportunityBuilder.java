...
/**
 * Builder for creating test Opportunity entities. Provides fluent API for setting opportunity
 * properties and sales stages.
 *
 * <p>This builder ensures all test opportunities are properly marked and can be identified for
 * cleanup.
 *
...
public class OpportunityBuilder {
  ...
  public OpportunityBuilder inStage(OpportunityStage stage) {
    this.stage = stage;
    // Always reset probability to stage default when stage changes
    // unless it was explicitly set via withProbability after this call
  ...
  /**
   * Builds an Opportunity entity WITHOUT persisting to database. Use this for unit tests or when
  ...
  public Opportunity build() {
    if (customer == null) {
      throw new IllegalStateException(
          "Customer is required for OpportunityBuilder. Use forCustomer() first.");
    }
    ...
    // Set expectedValue - use a default positive value if not set to avoid validation errors
    if (expectedValue == null) {
      expectedValue = BigDecimal.valueOf(10000); // Default value to avoid negative validation
    }
    opportunity.setExpectedValue(expectedValue);
    opportunity.setExpectedCloseDate(expectedCloseDate);

    // Set probability - use current value (either from withProbability or inStage)
    // If still null (shouldn't happen), use stage default
    if (probability == null) {
  ...
  /**
   * Builds and persists an Opportunity entity to the database. Use this for integration tests that
   * need DB interaction.
   *
  ...
  public Opportunity persist() {
    Opportunity opportunity = build(); // build() now automatically resets
