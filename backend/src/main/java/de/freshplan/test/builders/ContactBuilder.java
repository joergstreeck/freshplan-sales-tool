...
/**
 * Builder for creating test CustomerContact entities.
 * Provides fluent API for setting contact properties and predefined roles.
 * 
 * This builder ensures all test contacts are properly associated with customers
 * and marked for cleanup after tests.
 * 
...
public class ContactBuilder {
    ...
    /**
     * Builds a CustomerContact entity WITHOUT persisting to database.
    ...
    public CustomerContact build() {
        if (customer == null) {
            throw new IllegalStateException("Customer is required for ContactBuilder. Use forCustomer() first.");
        }
    ...
    /**
     * Builds and persists a CustomerContact entity to the database.
    ...
    public CustomerContact persist() {
        ...
        contactRepository.flush(); // Force immediate constraint validation
        return contact;
    }
}