...
/**
 * Builder for creating test CustomerTimelineEvent entities.
 * Provides fluent API for creating various timeline events for customer history.
 * 
 * This builder ensures all test timeline events are properly marked
...
public class TimelineEventBuilder {
    ...
    @Inject
    CustomerTimelineEventRepository repository;
    ...
    /**
     * Builds a CustomerTimelineEvent entity WITHOUT persisting to database.
    ...
    public CustomerTimelineEvent build() {
        if (customer == null) {
            throw new IllegalStateException("Customer is required for TimelineEventBuilder. Use forCustomer() first.");
        }
    ...
    /**
     * Builds and persists a CustomerTimelineEvent entity to the database.
