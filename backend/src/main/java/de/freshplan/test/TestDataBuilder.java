 ...
 *
 * <p>This class serves as the single entry point for all test data builders, providing a clean and
 * consistent API for creating test entities.
 *
 * <h3>Architecture</h3>
 *
 ...
 *
 * <ul>
 *   <li>This facade class as the central entry point
 *   <li>Individual builder classes for each entity type
 *   <li>Utility classes for common functionality
 *   <li>All test data is marked for easy identification and cleanup
 * </ul>
 *
 * <h3>Usage Example</h3>
 *
 ...
 *
 * <ul>
 *   <li>All test data is marked with isTestData=true (where available)
 *   <li>All test data has identifying prefixes ([TEST] or test-)
 *   <li>Builders provide both build() and persist() methods
 *   <li>build() creates objects without DB interaction
 *   <li>persist() creates and saves to DB in a transaction
 * </ul>
 *
...
public class TestDataBuilder {
  ...
  @Inject private OpportunityBuilder opportunityBuilder;

  @Inject private TimelineEventBuilder timelineEventBuilder;

  @Inject private UserBuilder userBuilder;

  /**
   * Creates a new CustomerBuilder for building test customers. The builder is reset to default
  ...
  /**
   * Creates a new ContactBuilder for building test contacts. The builder is reset to default values
  ...
  /**
   * Creates a new OpportunityBuilder for building test opportunities. The builder is reset to
   * default values before being returned.
   *
  ...
  /**
   * Creates a new TimelineEventBuilder for building test timeline events. The builder is reset to
   * default values before being returned.
   *
  ...
  /**
   * Creates a new UserBuilder for building test users. The builder is reset to default values
   * before being returned.
   *
  ...
  /**
   * Creates complex test scenarios with multiple related entities. This method is useful for
  ...
  /**
   * Builder for creating complex test scenarios with multiple related entities. Provides
   * convenience methods for setting up complete test environments.
   */
  public static class ScenarioBuilder {
     ...
     */
    public CompleteCustomerScenario completeCustomerWithContactsAndOpportunities(
        String companyName) {
      // Create customer
      Customer customer =
          testDataBuilder.customer().withCompanyName(companyName).asNewLead().persist();
      ...
      CustomerTimelineEvent firstContact =
          testDataBuilder
              .timelineEvent()
              .forCustomer(customer)
      ...
      return new CompleteCustomerScenario(
          customer, List.of(ceo, purchaser), opportunity, List.of(firstContact, meeting));
    }
