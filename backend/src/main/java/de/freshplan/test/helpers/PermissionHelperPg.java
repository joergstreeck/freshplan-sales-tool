 ...
 *
 * <p>Uses PostgreSQL ON CONFLICT clause to ensure atomic "INSERT OR SELECT" operations, preventing
 * race conditions when multiple tests try to create the same permission simultaneously (e.g., in
 * parallel CI execution).
 *
 * <h3>Why this approach?</h3>
 *
 * <ul>
 *   <li>No exception flows - always returns a valid Permission
 *   <li>100% thread-safe at database level
 *   <li>Atomic operation - no separate INSERT and SELECT
 *   <li>Works perfectly with parallel test execution
 * </ul>
 *
 * <h3>Usage Example</h3>
 *
...
public class PermissionHelperPg {
   ...
   *
   * <p>This method is completely race-condition-safe. If multiple threads/tests call this method
   * with the same permission code simultaneously, all will receive the same Permission instance
   * without any errors.
   *
   * <p>The description is prefixed with [TEST] to identify test-created permissions.
   *
  ...
  public Permission findOrCreatePermission(String code, String description) {
    ...
    if (parts.length != 2) {
      throw new IllegalArgumentException("Permission code must have exactly one ':', got: " + code);
    }
    ...
    String testDescription =
        description != null ? "[TEST] " + description : "[TEST] Auto-created permission";
  ...
  /**
   * Create multiple permissions atomically. Useful for setting up test scenarios that need multiple
   * permissions.
   *
  ...
  /**
   * Delete test permissions created by this helper. Only deletes permissions with [TEST] prefix in
   * description.
   *
   * <p>This is useful for cleanup in integration tests that don't use @TestTransaction.
   *
