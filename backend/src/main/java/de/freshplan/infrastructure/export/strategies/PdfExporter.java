...
/**
 * PDF export strategy using OpenPDF library.
...
public class PdfExporter implements ExportStrategy {
    ...
    public ExportResult export(List<?> data, ExportConfig config) {
        ...
        try {
            ...
            document.addAuthor(config.getGeneratedBy() != null ? config.getGeneratedBy() : "FreshPlan System");
    ...
    private void addHeader(Document document, ExportConfig config) throws DocumentException {
        ...
        metadata.add(new Chunk(LocalDateTime.now().format(
            DateTimeFormatter.ofPattern("dd.MM.yyyy HH:mm")), metaFont));
    ...
    private void addDataTable(Document document, List<?> data, ExportConfig config) throws DocumentException {
    ...
    private void addFooter(Document document, ExportConfig config) throws DocumentException {
        ...
        if (config.getFilters() != null && !config.getFilters().isEmpty()) {
            summary.add(new Chunk("Angewendete Filter: ", 
                new Font(Font.HELVETICA, 10, Font.BOLD)));
            for (Map.Entry<String, Object> filter : config.getFilters().entrySet()) {
                summary.add(new Chunk(String.format("%s=%s ", 
                    filter.getKey(), filter.getValue()), summaryFont));
            }
        }
    ...
    public Object extractFieldValue(Object record, String fieldKey) {
        ...
        try {
            ...
            try {
                ...
                // Try getter method
                String getterName = "get" + fieldKey.substring(0, 1).toUpperCase() + 
        ...
        switch (field.getType()) {
            ...
            case DATETIME:
                if (value instanceof LocalDateTime) {
                    String pattern = field.getFormat() != null ? 
                        field.getFormat() : "dd.MM.yyyy HH:mm";
