...
/**
 * CSV export strategy using OpenCSV library.
 * Combines the power of OpenCSV with our Universal Export Framework.
 * 
    ...
    public ExportResult export(List<?> data, ExportConfig config) {
        ...
        StreamingOutput stream = output -> {
            try (OutputStreamWriter writer = new OutputStreamWriter(output, StandardCharsets.UTF_8);
                ...
                // Build header
                String[] header = config.getVisibleFields().stream()
                ...
                for (Object record : data) {
                    String[] row = config.getVisibleFields().stream()
                        .map(field -> {
