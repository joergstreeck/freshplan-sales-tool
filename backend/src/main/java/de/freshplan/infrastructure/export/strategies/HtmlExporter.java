 ...
 * Generates HTML reports that can be displayed in browser or printed as PDF.
 * Uses FreshPlan CI colors and responsive design.
 * 
...
public class HtmlExporter implements ExportStrategy {
    ...
    private String generateStyles(ExportConfig.ExportStyles styles) {
        ...
        css.append("  background: linear-gradient(135deg, ")
           .append(styles.getPrimaryColor()).append(" 0%, ")
           .append(adjustBrightness(styles.getPrimaryColor(), 1.2)).append(" 100%);\n");
        ...
        css.append(".summary h2 { color: ").append(styles.getPrimaryColor()).append("; margin-bottom: 15px; }\n");
        css.append(".summary-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 20px; }\n");
        css.append(".summary-item { }\n");
        css.append(".summary-label { font-weight: 600; color: #666; }\n");
        css.append(".summary-value { font-size: 24px; color: ").append(styles.getPrimaryColor()).append("; }\n");
        ...
        css.append("  th { background-color: ").append(styles.getSecondaryColor()).append(" !important; }\n");
        ...
        styles.getCustomCss().forEach((selector, rules) -> {
    ...
    private String generateHeader(ExportConfig config) {
        ...
        if (config.getSubtitle() != null) {
            header.append("<p class=\"subtitle\">").append(escapeHtml(config.getSubtitle())).append("</p>\n");
        }
        
        header.append("<div class=\"metadata\">\n");
        header.append("Erstellt am: ").append(LocalDateTime.now().format(
    ...
    private String generateSummary(List<?> data, ExportConfig config) {
        ...
        if (!config.getFilters().isEmpty()) {
            ...
            summary.append("<div class=\"summary-value\">").append(config.getFilters().size()).append("</div>\n");
            summary.append("</div>\n");
        }
    ...
    private String generateFooter(ExportConfig config) {
        ...
        footer.append("<p>Dieser Bericht wurde automatisch generiert und ist nur für den internen Gebrauch bestimmt.</p>\n");
