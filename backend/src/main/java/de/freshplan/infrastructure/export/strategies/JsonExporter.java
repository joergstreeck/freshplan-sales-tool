...
/**
 * JSON export strategy using Jackson. Creates professional JSON exports with metadata and
 * formatting.
 *
    ...
    try {
      ...
      if (config.getFields() != null && !config.getFields().isEmpty()) {
        ...
        List<Map<String, Object>> filteredData =
            data.stream().map(record -> filterRecord(record, config.getVisibleFields())).toList();
        ...
        switch (field.getType()) {
          ...
          case DATETIME:
            if (value instanceof LocalDateTime) {
              String format =
                  field.getFormat() != null ? field.getFormat() : "yyyy-MM-dd'T'HH:mm:ss";
              value = ((LocalDateTime) value).format(DateTimeFormatter.ofPattern(format));
            }
            break;
          case CURRENCY:
