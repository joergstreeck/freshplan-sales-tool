 ...
 * 
 * <p>Provides synchronous and asynchronous event publishing capabilities
 * for decoupling domain operations from their side effects.
 * 
 * <p>Uses Quarkus CDI Events under the hood for reliable event delivery.
 * In production, this could be extended to use external message brokers
 * like Kafka or RabbitMQ for cross-service communication.
 */
@ApplicationScoped
public class EventBus {
    ...
    @Inject
    Event<Object> eventProducer;

    /**
     * Publishes an event synchronously.
     * 
     * All event handlers will be executed in the same transaction
     * as the caller. Use this for critical side effects that must
     * succeed together with the main operation.
     *
     ...
     * 
     * Event handlers will be executed in separate transactions.
     * Use this for non-critical side effects like analytics, notifications,
     * or cache updates that shouldn't block the main operation.
     *
     * @param event The domain event to publish
     */
    ...
    public void publishWithQualifier(Object event, java.lang.annotation.Annotation qualifier) {
        LOG.debug("Publishing qualified event: {} with qualifier: {}", 
            event.getClass().getSimpleName(), qualifier.annotationType().getSimpleName());
        try {
