...
/**
 * Bootstrap guard that ensures RLS is properly configured on application startup.
 * In production environments, this guard will fail-fast if RLS is not enabled.
 *
 * This is a critical security component that prevents accidental deployment
 * of the application without proper RLS enforcement.
 */
@Startup
@ApplicationScoped
public class RlsBootstrapGuard {
    ...
    void onStart(@Observes StartupEvent event) {
        LOG.infof("RLS Bootstrap Guard starting - Profile: %s, RLS Enabled: %s, Fail-Closed: %s",
                 profile, rlsEnabled, failClosed);

        // In production, RLS MUST be enabled
        if ("prod".equals(profile) && !rlsEnabled) {
            String error = "CRITICAL SECURITY ERROR: RLS interceptor is disabled in production environment!";
            LOG.fatal(error);
        ...
        if ("prod".equals(profile) && !failClosed) {
            String error = "CRITICAL SECURITY ERROR: RLS fail-closed mode is disabled in production environment!";
            LOG.fatal(error);
            throw new IllegalStateException(error);
        }
