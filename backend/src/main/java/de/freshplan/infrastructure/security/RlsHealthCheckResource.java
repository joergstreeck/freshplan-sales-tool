...
/**
 * Health check endpoint for RLS validation.
 * Provides monitoring capabilities to ensure RLS is properly configured.
 */
@Path("/health/rls")
@ApplicationScoped
public class RlsHealthCheckResource {
    ...
    public Response checkRlsHealth() {
        ...
        try {
            ...
            Long rowsWithoutGuc = (Long) em.createNativeQuery(
            ...
            // Overall status
            boolean healthy = rlsEnabled &&
                            (Boolean) contextResult[4] &&
                            activeConnections < 80 && // Connection pool threshold
                            rowsWithoutGuc == 0;
