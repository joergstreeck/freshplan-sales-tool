...
/**
 * Entity for Settings Registry (Sprint 1.2 PR #2).
 * Maps to the security_settings table with ETag support.
 */
@Entity
...
@NamedQueries({
    @NamedQuery(
        ...
        query = "SELECT s FROM Setting s WHERE s.scope = :scope " +
                "AND (:scopeId IS NULL AND s.scopeId IS NULL OR s.scopeId = :scopeId) " +
                "AND s.key = :key"
    ),
    @NamedQuery(
        name = "Setting.findByEtag",
        query = "SELECT s FROM Setting s WHERE s.etag = :etag"
    ),
    @NamedQuery(
...
public class Setting extends PanacheEntityBase {
    ...
    public static Setting findByScopeAndKey(SettingsScope scope, String scopeId, String key) {
        return find("#Setting.findByScopeAndKey",
    ...
    public static Setting findByEtag(String etag) {
        return find("#Setting.findByEtag", Parameters.with("etag", etag))
            .firstResult();
    }
     ...
     */
    public static Setting resolveHierarchical(String key, String tenantId, String territory,
                                             String accountId, String contactRole) {
        return find("#Setting.resolveHierarchy",
    ...
    public String getCacheKey() {
        return String.format("setting:%s:%s:%s",
            scope, scopeId != null ? scopeId : "null", key);
    }
}