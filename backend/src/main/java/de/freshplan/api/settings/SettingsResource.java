...
/**
 * REST API for Settings Registry (Sprint 1.2 PR #2). Provides ETag-based caching and hierarchical
 * settings resolution.
 */
@Path("/api/settings")
...
public class SettingsResource {
  ...
  /**
   * Gets a specific setting by scope and key. Supports ETag-based caching with If-None-Match
   * header.
   */
  @GET
  @Operation(
      summary = "Get a setting",
      description = "Retrieves a setting by scope and key with ETag support")
  @APIResponses({
    @APIResponse(
        responseCode = "200",
  ...
  public Response getSetting(
      @QueryParam("scope") @Parameter(required = true, description = "Setting scope")
          SettingsScope scope,
      @QueryParam("scopeId")
          @Parameter(description = "Scope identifier (e.g., tenant ID, territory code)")
          String scopeId,
      @QueryParam("key") @Parameter(required = true, description = "Setting key") String key,
      @HeaderParam(IF_NONE_MATCH_HEADER) @Parameter(description = "ETag for conditional request")
          String ifNoneMatch) {
    LOG.infof(
        "GET setting: scope=%s, scopeId=%s, key=%s, ifNoneMatch=%s",
    ...
    return Response.ok(dto).header(ETAG_HEADER, s.etag).cacheControl(getCacheControl()).build();
  }
  ...
  @Path("/resolve/{key}")
  @Operation(
      summary = "Resolve setting hierarchically",
  ...
  public Response resolveSetting(
    ...
    LOG.infof("Resolving setting: key=%s, tenant=%s, territory=%s", key, tenantId, territory);

    var context = new SettingsService.SettingsContext(tenantId, territory, accountId, contactRole);
  ...
  public Response getSettingById(
      @PathParam("id") UUID id, @HeaderParam(IF_NONE_MATCH_HEADER) String ifNoneMatch) {
  ...
  /** Creates or updates a setting. Uses If-Match header for optimistic locking on updates. */
  @PUT
  @Path("/{id}")
  @RolesAllowed("admin")
  @Transactional
  @Operation(
      ...
      description = "Updates a setting with optimistic locking")
  @APIResponses({
  ...
  public Response updateSetting(
    ...
    try {
      Setting updated =
          settingsService.updateSettingWithEtag(id, update.value, update.metadata, ifMatch, userId);

      SettingDto dto = SettingDto.from(updated);
      return Response.ok(dto).header(ETAG_HEADER, updated.etag).build();
    ...
    try {
      Setting created =
          settingsService.createSettingStrict(
              create.scope, create.scopeId, create.key, create.value, create.metadata, userId);

      URI location = uriInfo.getAbsolutePathBuilder().path(created.id.toString()).build();

      SettingDto dto = SettingDto.from(created);
      return Response.created(location).entity(dto).header(ETAG_HEADER, created.etag).build();
    } catch (WebApplicationException e) {
  ...
  public Response listSettings(
    ...
    List<SettingDto> dtos = settings.stream().map(SettingDto::from).toList();

    return Response.ok(dtos).cacheControl(getCacheControl()).build();
  }
  ...
  public record SettingDto(
      ...
      @Schema(description = "Setting scope", required = true) SettingsScope scope,
      @Schema(description = "Scope identifier") String scopeId,
      @Schema(description = "Setting key", required = true) String key,
      @Schema(description = "Setting value as JSON", required = true, type = SchemaType.OBJECT)
          JsonObject value,
      @Schema(description = "Additional metadata", type = SchemaType.OBJECT) JsonObject metadata,
      @Schema(description = "Entity tag for caching") String etag,
      @Schema(description = "Version number for optimistic locking") Integer version,
      @Schema(description = "Last update timestamp") String updatedAt) {
