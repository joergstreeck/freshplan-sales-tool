 ...
 *
 * <p>Bietet Endpoints für: - Kontextuelle Hilfe-Anfragen - User Feedback Collection - Help Content
 * Search - Analytics und Monitoring
 */
@Path("/api/help")
...
public class HelpSystemResource {
  ...
  @Inject HelpContentService helpService;
  ...
  public Response getHelpForFeature(
    ...
    try {
      LOG.debug("Help request for feature: {} by user: {} (level: {})", feature, userId, userLevel);
      ...
      HelpRequest request =
          HelpRequest.builder()
              ...
              .sessionId(sessionId)
              .context(
                  Map.of("timestamp", System.currentTimeMillis(), "endpoint", "getHelpForFeature"))
              .build();
  ...
  public Response submitFeedback(FeedbackRequest request) {
    try {
      LOG.info("Feedback submitted for help {}: helpful={}", request.helpId, request.helpful);

      helpService.recordFeedback(
          request.helpId, request.userId, request.helpful, request.timeSpent, request.comment);
  ...
  public Response searchHelp(
    ...
    try {
      ...
      LOG.debug("Help search: '{}' by user: {} (level: {})", searchTerm, userId, userLevel);

      List<HelpResponse> results =
          helpService.searchHelp(
              searchTerm.trim(), userLevel, userRoles != null ? userRoles : List.of());
  ...
  public Response getFeatureAnalytics(@PathParam("feature") String feature) {
    try {
      ...
      return Response.ok(
              Map.of("feature", feature, "message", "Feature analytics not yet implemented"))
          .build();
  ...
  public Response getHealthStatus() {
    try {
      ...
      return Response.ok(new HealthStatus("Help System operational", totalViews, activeContent))
          .build();
  ...
  public Response createHelpContent(CreateHelpContentRequest request) {
    try {
      ...
      var content =
          helpService.createOrUpdateHelpContent(
              request.feature,
              helpType,
              request.title,
      ...
      return Response.status(Response.Status.CREATED)
          .entity(Map.of("id", content.id, "message", "Help content created successfully"))
          .build();
  ...
  public Response toggleHelpContent(
    ...
    try {
      helpService.toggleHelpContent(helpId, active, updatedBy != null ? updatedBy : "system");

      return Response.ok(
              new SuccessResponse(
                  String.format(
                      "Help content %s successfully", active ? "activated" : "deactivated")))
          .build();
