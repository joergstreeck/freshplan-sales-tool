...
public class ExportResource {
  ...
  @Inject ExportServiceV2 exportService;
  ...
  /**
   * Export audit data as HTML (for PDF printing) UPDATED: Returns HTML that can be printed to PDF
   * in browser
   */
  @GET
  @Path("/audit/pdf")
  ...
  public Response exportAuditExcel(
    ...
    String filename =
        String.format(
            "audit_analysis_%s.xlsx", LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE));
        ...
        output -> {
          try (BufferedWriter writer =
            ...
            for (Customer customer : customers) {
              if (includeContacts
                  && customer.getContacts() != null
                  && !customer.getContacts().isEmpty()) {
  ...
  public Response exportCustomersExcel(
    ...
    String filename =
        String.format(
            "customers_%s.xlsx", LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE));
   ...
   *
   * <p>This is a robust solution that generates HTML which can be: 1. Displayed in the browser 2.
   * Printed to PDF using browser's print function 3. Saved as HTML for further processing
   *
   * <p>This approach avoids PDF library issues and provides more flexibility
   */
  @GET
  @Path("/customers/pdf")
  @RolesAllowed({"admin", "manager", "sales"})
  ...
  /**
   * Export compliance report as HTML (for PDF printing) UPDATED: Returns HTML that can be printed
   * to PDF in browser
   */
  @POST
  @Path("/compliance/pdf")
