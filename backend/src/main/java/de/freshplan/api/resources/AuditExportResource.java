...
public class AuditExportResource {
    ...
    public Response exportAudit(
        ...
        try {
            ...
            var criteria = AuditRepository.AuditSearchCriteria.builder()
            ...
            ExportConfig.Builder configBuilder = ExportConfig.builder()
                    ...
                    .subtitle(String.format("Von %s bis %s", 
            ...
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("timestamp").label("Zeitstempel").type(ExportConfig.FieldType.DATETIME).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("eventType").label("Ereignistyp").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("entityType").label("Entitätstyp").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("entityId").label("Entitäts-ID").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("userId").label("Benutzer-ID").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("userName").label("Benutzer").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("userRole").label("Rolle").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("source").label("Quelle").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("ipAddress").label("IP-Adresse").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("userAgent").label("User Agent").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("changeReason").label("Änderungsgrund").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("oldValue").label("Alter Wert").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("newValue").label("Neuer Wert").type(ExportConfig.FieldType.STRING).build());
            configBuilder.addField(ExportConfig.FieldConfig.builder()
                .key("userComment").label("Kommentar").type(ExportConfig.FieldType.STRING).build());
            ...
            if (eventTypes != null && !eventTypes.isEmpty()) {
                filters.put("eventTypes", String.join(", ", 
                    eventTypes.stream().map(Enum::toString).toList()));
            }
            if (!filters.isEmpty()) {
