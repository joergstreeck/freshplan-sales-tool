 ...
 *
 * <p>Bietet Endpoints für: - Dashboard Data - Cost Statistics - Budget Management - Alert
 * Triggering
 */
...
public class CostManagementResource {
  ...
  @Inject CostTrackingService costService;
  ...
  public Response getCostStatistics(
      @QueryParam("start") String startStr, @QueryParam("end") String endStr) {
  ...
  public Response createBudgetLimit(BudgetLimitRequest request) {
    try {
      ...
      BudgetLimit limit =
          costService.createOrUpdateBudgetLimit(
              request.scope,
              request.scopeValue,
              ...
              new BigDecimal(
                  request.hardStopThreshold != null ? request.hardStopThreshold : "0.95"));
  ...
  public Response simulateTransaction(SimulateTransactionRequest request) {
    try {
      LOG.info(
          "Simulating cost transaction: {} {} for €{}",
          request.service,
          request.feature,
      ...
      try (var context =
        ...
        return Response.ok()
            .entity(
                "{\"message\": \"Transaction simulated successfully\", \"transactionId\": \""
