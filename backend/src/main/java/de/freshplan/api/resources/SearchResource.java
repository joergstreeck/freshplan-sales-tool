...
/**
 * REST resource for universal search functionality across customers and contacts.
 * Provides intelligent query analysis and relevance-based results.
 * 
 * @since FC-005 PR4
 */
@Path("/api/search")
...
public class SearchResource {
     ...
     * Analyzes the query to determine search type (email, phone, customer number, or text)
    ...
    public Response universalSearch(
            ...
            @Parameter(
                description = "Include contacts in results",
                example = "true"
            )
        ...
        LOG.infof("Universal search request: query='%s', includeContacts=%s, includeInactive=%s, limit=%d", 
        ...
        try {
            ...
            LOG.infof("Search completed: %d customers, %d contacts found", 
                    results.getCustomers().size(), 
                    results.getContacts().size());
    ...
    /**
     * Quick search endpoint for autocomplete functionality.
     * Returns only top matches with minimal data.
     * 
