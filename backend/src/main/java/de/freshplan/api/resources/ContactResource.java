...
/**
 * REST API endpoints for Contact management. Provides CRUD operations for contacts with
...
public class ContactResource {
  ...
  public Response checkEmailAvailability(
      @Parameter(description = "Email to check", required = true) @QueryParam("email") String email,
      @Parameter(description = "Contact ID to exclude from check") @QueryParam("excludeId")
          UUID excludeId) {
    boolean inUse = contactService.isEmailInUse(email, excludeId);
    return Response.ok().entity(new EmailCheckResult(email, !inUse)).build();
  }
