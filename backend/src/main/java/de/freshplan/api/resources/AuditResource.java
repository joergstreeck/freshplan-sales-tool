...
import de.freshplan.domain.audit.service.dto.AuditExportFormat;
...
public class AuditResource {
    ...
    @APIResponse(
        ...
        content = @Content(
            schema = @Schema(implementation = AuditEntry.class, type = SchemaType.ARRAY)
        )
    )
    public Response getEntityAuditTrail(
        ...
        auditService.logAsync(AuditContext.of(
    ...
    public Response searchAuditTrail(
        ...
        Instant from = fromStr != null ? Instant.parse(fromStr) : Instant.now().minusSeconds(86400 * 7); // Default 7 days
        ...
        if ("json".equalsIgnoreCase(format)) {
            ...
            StreamingOutput stream = output -> {
                try (BufferedWriter writer = new BufferedWriter(new OutputStreamWriter(output));
                    ...
                    // Write CSV header
                    writer.write("Timestamp,Event Type,Entity Type,Entity ID,User,User Role,Change Reason,IP Address,Source\n");
                    ...
                    entries.forEach(entry -> {
                        try {
                            ...
                            log.error("Error writing audit entry to CSV", e);
                        }
                    });
    ...
    public Response verifyIntegrity(
        ...
        if (issues.isEmpty()) {
            return Response.ok(Map.of("status", "valid", "message", "Audit trail integrity verified")).build();
        } else {
    ...
    private static final java.util.Map<String, Object> Map = java.util.Map;
}