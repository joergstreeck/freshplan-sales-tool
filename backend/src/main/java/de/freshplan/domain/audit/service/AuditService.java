...
public class AuditService {
    ...
    @Inject
    Event<AuditEvent> auditEventBus;
    ...
    void init() {
        ...
        log.info("Audit Service initialized with {} async threads", 
                configuration.getAsyncThreadPoolSize());
    }
    ...
    public CompletableFuture<UUID> logAsync(
        ...
        return logAsync(AuditContext.builder()
                .eventType(eventType)
    ...
    public CompletableFuture<UUID> logAsync(AuditContext context) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                log.error("Failed to log audit event: {}", context.getEventType(), e);
    ...
    public UUID logSync(AuditContext context) {
        try {
            ...
            log.debug("Audit event logged: {} for {}/{}", 
    ...
    public UUID logSecurityEvent(AuditEventType eventType, String details) {
        return logSync(AuditContext.builder()
                .eventType(eventType)
                .entityType("SECURITY")
    ...
    private AuditEntry buildAuditEntry(AuditContext context) {
        ...
        AuditEntry.AuditEntryBuilder builder = AuditEntry.builder()
                ...
                .userName(context.getUserName() != null ? context.getUserName() : securityUtils.getCurrentUserName())
                .userRole(context.getUserRole() != null ? context.getUserRole() : securityUtils.getCurrentUserRole())
                ...
                .sessionId(context.getSessionId() != null ? context.getSessionId() : securityUtils.getCurrentSessionId())
        ...
        if (httpRequest != null && context.getIpAddress() == null) {
    ...
    private String getUserAgent() {
        return httpRequest != null ? httpRequest.getHeader("User-Agent") : "SYSTEM";
    }
    ...
    private void notifySecurityTeam(AuditEntry entry) {
        ...
        log.warn("Security notification required for event: {} - {}", 
                entry.getEventType(), 
                entry.getId());
    }
    ...
    private void logToFallback(AuditContext context, Exception error) {
        ...
        log.error("AUDIT_FALLBACK: {} {} {} - Error: {}", 
                 context.getEventType(),
                 context.getEntityType(),
                 context.getEntityId(),
                 error.getMessage());
