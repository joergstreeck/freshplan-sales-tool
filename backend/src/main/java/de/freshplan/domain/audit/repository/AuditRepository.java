 ...
 * 
 * Provides comprehensive query capabilities for compliance,
 * security monitoring, and audit trail analysis.
 * 
...
public class AuditRepository implements PanacheRepositoryBase<AuditEntry, UUID> {
    ...
    public List<AuditEntry> findByUser(UUID userId, Instant from, Instant to) {
        return find("userId = ?1 and timestamp between ?2 and ?3",
                   Sort.descending("timestamp"),
                   userId, from, to)
                .list();
    }
    ...
    public List<AuditEntry> findByEventType(AuditEventType eventType, Instant from, Instant to) {
        return find("eventType = ?1 and timestamp between ?2 and ?3",
                   Sort.descending("timestamp"),
                   eventType, from, to)
                .list();
    }
    ...
    public List<AuditEntry> findSecurityEvents(Instant from, Instant to) {
        return find("eventType in ?1 and timestamp between ?2 and ?3",
                   Sort.descending("timestamp"),
                   getSecurityEventTypes(), from, to)
                .list();
    }
    ...
    public List<AuditEntry> findFailures(Instant from, Instant to) {
        return find("(eventType like '%FAILURE%' or eventType like '%DENIED%' or eventType like '%ERROR%') " +
    ...
    public Stream<AuditEntry> streamForExport(AuditSearchCriteria criteria) {
        ...
        TypedQuery<AuditEntry> query = getEntityManager()
    ...
    public List<AuditEntry> findRequiringNotification() {
        List<AuditEventType> notificationTypes = Arrays.stream(AuditEventType.values())
                .filter(AuditEventType::requiresNotification)
                .toList();
        
        return find("eventType in ?1 and timestamp > ?2",
                   Sort.descending("timestamp"),
                   notificationTypes,
    ...
    public List<AuditIntegrityIssue> verifyIntegrity(Instant from, Instant to) {
        ...
        List<AuditEntry> entries = find("timestamp between ?1 and ?2",
    ...
    public static class AuditStatistics {
        ...
        public static class AuditStatisticsBuilder {
            public AuditStatisticsBuilder period(Instant from, Instant to) {
