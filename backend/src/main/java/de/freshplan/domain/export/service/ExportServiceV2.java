 ...
 * UPDATED: Removed iTextPDF dependency, using HtmlExportService for PDF generation
 * 
...
public class ExportServiceV2 {
    ...
    /**
     * Generate HTML report for audit trail (to be printed as PDF)
     * UPDATED: Returns HTML instead of PDF bytes
     */
    public String generateAuditHtml(ExportRequest request) {
        ...
        html.append("<p class='metadata'>Generated: ").append(LocalDateTime.now()
                .format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)).append("</p>");
        
        if (request.getEntityType() != null) {
            html.append("<p class='metadata'>Entity Type: ").append(escapeHtml(request.getEntityType())).append("</p>");
        }
        html.append("</div>");
        ...
        for (AuditEntry entry : entries) {
            ...
            html.append("<td>").append(escapeHtml(entry.getUserName() != null ? entry.getUserName() : "System")).append("</td>");
            html.append("<td>").append(escapeHtml(entry.getEntityType())).append("/").append(entry.getEntityId()).append("</td>");
     ...
     * UNCHANGED: Excel generation still uses Apache POI
     */
    ...
    /**
     * Generate HTML report for compliance (to be printed as PDF)
     * UPDATED: Returns HTML instead of PDF bytes
     */
    public String generateComplianceHtml(ExportOptions options) {
        ...
        html.append("<head>");
        html.append("<meta charset='UTF-8'>");
        html.append("<title>Compliance Report - ").append(escapeHtml(options.getReportType())).append("</title>");
        ...
        // Header
        html.append("<div class='header'>");
        html.append("<h1>Compliance Report - ").append(escapeHtml(options.getReportType())).append("</h1>");
        html.append("<p class='metadata'>Generated: ").append(LocalDateTime.now()
                .format(DateTimeFormatter.ISO_LOCAL_DATE_TIME)).append("</p>");
        ...
        if ("GDPR".equals(options.getReportType())) {
            ...
            html.append("<li>Data Modifications: ")
                .append(entries.stream().filter(e -> "UPDATE".equals(e.getEventType().toString())).count())
                .append("</li>");
            html.append("</ul>");
            html.append("</div>");
        }
