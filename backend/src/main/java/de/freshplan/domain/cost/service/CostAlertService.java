...
public class CostAlertService {
  ...
  public void sendBudgetAlert(BudgetLimit limit, BigDecimal currentUsage) {
    ...
    LOG.warn(
        "BUDGET ALERT [{}] - {}: {}% (€{}/€{}) - Aktueller Verbrauch: €{}",
        alertLevel,
  ...
  public void sendOverBudgetAlert(CostTransaction transaction) {
    ...
    BigDecimal percentageOver =
        difference
            .divide(transaction.estimatedCost, 4, RoundingMode.HALF_UP)
            .multiply(BigDecimal.valueOf(100));

    LOG.warn(
        "COST OVERRUN - Transaction {}: {} {} kostet €{} statt geschätzt €{} (+{}%)",
        transaction.id,
        transaction.service,
        transaction.feature,
  ...
  public void sendDailySmary(BigDecimal totalCost, BigDecimal budgetLimit, int transactionCount) {
    if (budgetLimit != null) {
      BigDecimal percentage =
          totalCost.divide(budgetLimit, 4, RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(100));

      LOG.info(
          "DAILY COST SUMMARY - Gesamt: €{} / €{} ({}%) - {} Transaktionen",
      ...
      LOG.info(
          "DAILY COST SUMMARY - Gesamt: €{} - {} Transaktionen (kein Budget-Limit gesetzt)",
          totalCost.setScale(2, RoundingMode.HALF_UP),
          transactionCount);
    }
