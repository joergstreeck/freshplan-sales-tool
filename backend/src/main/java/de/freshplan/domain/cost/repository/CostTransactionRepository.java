...
public class CostTransactionRepository implements PanacheRepositoryBase<CostTransaction, UUID> {
  ...
  public BigDecimal getTotalCostByService(String service, LocalDateTime start, LocalDateTime end) {
    return find(
            "SELECT COALESCE(SUM(actualCost), 0) FROM CostTransaction t "
  ...
  public List<Object[]> getTopCostDriversByService(
      LocalDateTime start, LocalDateTime end, int limit) {
    return getEntityManager()
        .createQuery(
            "SELECT t.service, SUM(t.actualCost), COUNT(t) "
  ...
  @SuppressWarnings("unchecked")
  public List<Object[]> getTopCostDriversByFeature(
      LocalDateTime start, LocalDateTime end, int limit) {
    return getEntityManager()
        .createQuery(
            "SELECT t.feature, SUM(t.actualCost), COUNT(t) "
                + "FROM CostTransaction t "
                + "WHERE t.startTime >= :start AND t.startTime < :end AND t.status = :status "
                + "GROUP BY t.feature "
                + "ORDER BY SUM(t.actualCost) DESC")
  ...
  public List<CostTransaction> findOverBudgetTransactions(LocalDateTime start, LocalDateTime end) {
    return find("startTime >= ?1 AND startTime < ?2 AND actualCost > estimatedCost", start, end)
        .list();
  }
  ...
  public List<Object[]> getAverageTokenCostsByService(LocalDateTime start, LocalDateTime end) {
    return getEntityManager()
        .createQuery(
            "SELECT t.service, t.model, AVG(t.actualCost / NULLIF(t.tokensUsed, 0)) "
                + "FROM CostTransaction t "
                + "WHERE t.startTime >= :start AND t.startTime < :end AND t.status = :status "
                + "AND t.tokensUsed > 0 "
