...
/**
 * Service for managing customer timeline events.
 * Handles creation, retrieval, and management of customer interaction history.
 * 
...
public class CustomerTimelineService {
    ...
    public TimelineEventResponse createEvent(
        ...
        event.setImportance(request.getImportance() != null ? 
                request.getImportance() : ImportanceLevel.MEDIUM);
    ...
    public TimelineEventResponse createNote(
            @NotNull UUID customerId,
            @Valid @NotNull CreateNoteRequest request) {
    ...
    public TimelineEventResponse createCommunication(
            @NotNull UUID customerId,
            @Valid @NotNull CreateCommunicationRequest request) {
        
        LOG.infof("Creating communication event for customer %s via %s", 
    ...
    public TimelineListResponse getCustomerTimeline(
        ...
        if (search != null && !search.isBlank()) {
            ...
            totalElements = timelineRepository
                    .count("customer.id = ?1 and isDeleted = false and " +
                          "(lower(title) like ?2 or lower(description) like ?2)",
                          customerId, "%" + search.toLowerCase() + "%");
        } else if (category != null && !category.isBlank()) {
            // Filter by category
            EventCategory eventCategory = EventCategory.valueOf(category);
            events = timelineRepository.findByCustomerIdAndCategory(
                    customerId, eventCategory, pageRequest);
            totalElements = timelineRepository.countByCustomerIdAndCategory(
                    customerId, eventCategory);
        } else {
            // Get all events
        ...
        List<TimelineEventResponse> content = events.stream()
    ...
    public List<TimelineEventResponse> getFollowUpEvents(@NotNull UUID customerId) {
        ...
        List<CustomerTimelineEvent> events = timelineRepository
                .findRequiringFollowUp(customerId);
    ...
    public List<TimelineEventResponse> getOverdueFollowUps(@NotNull UUID customerId) {
        ...
        List<CustomerTimelineEvent> events = timelineRepository
                .findOverdueFollowUps(customerId);
     ...
     */
    public List<TimelineEventResponse> getRecentCommunications(
            @NotNull UUID customerId, 
            int days) {
        
        LOG.debugf("Getting recent communications for customer %s (last %d days)", 
                customerId, days);
        
        List<CustomerTimelineEvent> events = timelineRepository
                .findRecentCommunications(customerId, days);
    ...
    public TimelineEventResponse updateEvent(
            @NotNull UUID eventId,
            @Valid @NotNull UpdateTimelineEventRequest request) {
        
        LOG.infof("Updating timeline event %s", eventId);
        
        CustomerTimelineEvent event = timelineRepository.findByIdOptional(eventId)
    ...
    public void createSystemEvent(
        ...
        LOG.debugf("Creating system event for customer %s: %s", 
                customer.getId(), eventType);
        
        CustomerTimelineEvent event = CustomerTimelineEvent.createSystemEvent(
                customer, eventType, description, performedBy);
