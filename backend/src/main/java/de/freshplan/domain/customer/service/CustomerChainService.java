...
/**
 * Service for managing customer chain structure and business model.
 * Implements Sprint 2 sales-focused features.
 * 
 * @since 2.0.0
 */
@ApplicationScoped
...
public class CustomerChainService {
    ...
    public ChainStructureDto updateChainStructure(
            UUID customerId, 
            ChainStructureDto chainStructure,
            String updatedBy) {
        ...
        if (!chainStructure.isValid()) {
            throw new IllegalArgumentException(
                "Sum of locations exceeds total EU locations"
            );
        }
    ...
    public PotentialCalculationResponse calculatePotential(
        ...
        log.info("Calculating potential for customer: {} in industry: {}", 
            customerId, request.getIndustry());
