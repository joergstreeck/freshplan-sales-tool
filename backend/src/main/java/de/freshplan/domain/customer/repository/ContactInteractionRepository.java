...
/**
 * Repository for ContactInteraction entities.
 * Provides methods for querying interaction history and analytics.
 */
@ApplicationScoped
public class ContactInteractionRepository implements PanacheRepositoryBase<ContactInteraction, UUID> {
    ...
    public List<ContactInteraction> findByContactPaginated(Contact contact, Page page) {
        return find("contact", Sort.by("timestamp").descending(), contact)
                .page(page)
                .list();
    }
    ...
    public List<ContactInteraction> findByContactAndType(Contact contact, InteractionType type) {
        return find("contact = :contact and type = :type", 
    ...
    public List<ContactInteraction> findRecentInteractions(Contact contact, int days) {
        ...
        return find("contact = :contact and timestamp >= :since",
                   Sort.by("timestamp").descending(),
    ...
    public Map<InteractionType, Long> countInteractionsByType(Contact contact) {
        return find("select i.type, count(i) from ContactInteraction i " +
    ...
    public List<ContactInteraction> findPendingFollowUps() {
        return find("nextActionDate is not null and nextActionDate <= :now",
     ...
     */
    public List<ContactInteraction> findBySentiment(Contact contact, 
                                                   Double minSentiment, 
                                                   Double maxSentiment) {
        return find("contact = :contact and sentimentScore between :min and :max",
                   Sort.by("timestamp").descending(),
                   Parameters.with("contact", contact)
                          .and("min", minSentiment)
                          .and("max", maxSentiment))
                .list();
    }
    ...
    public Double calculateAverageSentiment(Contact contact) {
        return find("select avg(i.sentimentScore) from ContactInteraction i " +
    ...
    public List<ContactInteraction> findCustomerInitiated(Contact contact) {
        return find("contact = :contact and initiatedBy = 'CUSTOMER'",
                   Sort.by("timestamp").descending(),
                   Parameters.with("contact", contact))
                .list();
    }
    ...
    public Double calculateResponseRate(Contact contact) {
        Long customerInitiated = count("contact = :contact and initiatedBy = 'CUSTOMER'",
                                     Parameters.with("contact", contact));
        
        if (customerInitiated == 0) return 100.0; // No customer inquiries to respond to
        
        Long salesResponses = count("contact = :contact and initiatedBy = 'SALES' " +
