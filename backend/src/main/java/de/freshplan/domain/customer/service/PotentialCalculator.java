...
/**
 * Calculator service for customer sales potential. Implements industry-specific calculations and
...
public class PotentialCalculator {
  ...
  public PotentialCalculationResponse calculate(
      Customer customer, PotentialCalculationRequest request) {
    ...
    BigDecimal yearlyPotential = monthlyPotential.multiply(new BigDecimal(MONTHS_PER_YEAR));
  ...
  private BigDecimal calculateHotelPotential(
      PotentialCalculationRequest request, List<QuickWinDto> quickWins) {
    ...
    if (request.getBooleanValue("breakfast", false)) {
      ...
      if (request.getBooleanValue("breakfastWarm", false)) {
        BigDecimal warmBonus =
            BREAKFAST_WARM_RATE
                .subtract(BREAKFAST_RATE)
                .multiply(new BigDecimal(guests))
                .multiply(new BigDecimal(WORKING_DAYS_MONTH));
        potential = potential.add(warmBonus);

        quickWins.add(
            new QuickWinDto("Warmes Frühstück optimieren", warmBonus, "Cook&Fresh® Frühstück")
                .withPriority(2));
      }
    }
    ...
    if (request.getBooleanValue("lunch", false)) {
      ...
      BigDecimal lunchPotential =
          LUNCH_RATE.multiply(new BigDecimal(covers)).multiply(new BigDecimal(WORKING_DAYS_MONTH));
    ...
    if (request.getBooleanValue("dinner", false)) {
      ...
      BigDecimal dinnerPotential =
          DINNER_RATE.multiply(new BigDecimal(covers)).multiply(new BigDecimal(WORKING_DAYS_MONTH));
    ...
    if (request.getBooleanValue("roomService", false)) {
      ...
      BigDecimal roomServicePotential =
          ROOMSERVICE_RATE
  ...
  private BigDecimal calculateRestaurantPotential(
    ...
    BigDecimal avgSpend = new BigDecimal(request.getIntegerValue("averageSpendPerCover", 25));
  ...
  private void addPainPointQuickWins(
      List<String> painPoints, List<QuickWinDto> quickWins, BigDecimal basePotential) {
