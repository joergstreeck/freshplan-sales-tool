...
/**
 * Repository for managing customer timeline events.
 * Provides comprehensive methods for querying and managing timeline history.
 * 
...
@ApplicationScoped
@Transactional
public class CustomerTimelineRepository implements PanacheRepositoryBase<CustomerTimelineEvent, UUID> {
    ...
    public List<CustomerTimelineEvent> findByCustomerIdAndImportance(
        ...
        return find("customer.id = :customerId and importance >= :importance and isDeleted = false",
    ...
    public List<CustomerTimelineEvent> findByCustomerIdAndDateRange(
        ...
        return find("customer.id = :customerId and eventDate between :startDate and :endDate and isDeleted = false",
    ...
    public List<CustomerTimelineEvent> findRequiringFollowUp(UUID customerId) {
        return find("customer.id = :customerId and requiresFollowUp = true " +
    ...
    public List<CustomerTimelineEvent> findRecentCommunications(UUID customerId, int days) {
        ...
        return find("customer.id = :customerId and category in :categories " +
                   "and eventDate >= :since and isDeleted = false",
                   Sort.by("eventDate").descending(),
                   Parameters.with("customerId", customerId)
                           .and("categories", List.of(
    ...
    public List<CustomerTimelineEvent> findByCustomerIdAndEventType(
        ...
        return find("customer.id = :customerId and eventType = :eventType and isDeleted = false",
                   Sort.by("eventDate").descending(),
                   Parameters.with("customerId", customerId)
    ...
    public List<CustomerTimelineEvent> findByCustomerIdAndTag(
        ...
        return find("customer.id = :customerId and tags like :tag and isDeleted = false",
                   Sort.by("eventDate").descending(),
                   Parameters.with("customerId", customerId)
                           .and("tag", "%" + tag + "%"))
                .page(page)
    ...
    public long countByCustomerIdAndCategory(UUID customerId, EventCategory category) {
        return count("customer.id = :customerId and category = :category and isDeleted = false",
                    Parameters.with("customerId", customerId)
                            .and("category", category));
    }
    ...
    public void softDelete(UUID eventId, String deletedBy) {
        update("isDeleted = true, deletedAt = :now, deletedBy = :deletedBy where id = :id",
              Parameters.with("now", LocalDateTime.now())
                      .and("deletedBy", deletedBy)
                      .and("id", eventId));
    }
    ...
    public void restore(UUID eventId) {
        update("isDeleted = false, deletedAt = null, deletedBy = null where id = :id",
    ...
    public void completeFollowUp(UUID eventId, String completedBy) {
        update("followUpCompleted = true, updatedBy = :completedBy, updatedAt = :now " +
    ...
    public List<CustomerTimelineEvent> findByPerformedBy(String performedBy, Page page) {
        return find("performedBy = :performedBy and isDeleted = false",
                   Sort.by("eventDate").descending(),
    ...
    public TimelineSummary getTimelineSummary(UUID customerId) {
        ...
        summary.communicationEvents = countByCustomerIdAndCategory(customerId, EventCategory.COMMUNICATION);
