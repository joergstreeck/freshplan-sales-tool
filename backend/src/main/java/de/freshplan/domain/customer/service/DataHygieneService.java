 ...
 * 
 * Implementiert automatische Datenalterung-Erkennung, proaktive Update-Kampagnen
 * und Data Quality Scoring basierend auf dem Data Strategy Intelligence Konzept.
 */
@ApplicationScoped
@Transactional
public class DataHygieneService {
    ...
    /**
     * Wöchentliche Überprüfung der Datenfreshness.
     * Läuft jeden Montag um 9:00 Uhr.
     * TODO: Aktiviere @Scheduled(cron = "0 0 9 * * MON") nach Hinzufügung der Scheduler-Dependency
     */
    ...
    public void checkDataFreshness() {
        ...
        try {
            ...
            List<Contact> criticalContacts = contactRepository
                .find("updatedAt < ?1", oneYearAgo)
                .list();
    ...
    /**
     * Tägliche Qualitätsbewertung für alle Kontakte.
     * Läuft jeden Tag um 2:00 Uhr nachts.
     * TODO: Aktiviere @Scheduled(cron = "0 0 2 * * *") nach Hinzufügung der Scheduler-Dependency
     */
    ...
    public DataQualityScore calculateDataQualityScore(Contact contact) {
        ...
        if (contact.getWarmthScore() == null || contact.getWarmthConfidence() == null || 
    ...
    private void processStaleContacts(List<Contact> staleContacts) {
        ...
        for (Contact contact : staleContacts) {
            LOG.debug("Stale contact found: {} (last updated: {})", 
    ...
    private void processCriticalContacts(List<Contact> criticalContacts) {
        for (Contact contact : criticalContacts) {
            LOG.warn("Critical contact found: {} (last updated: {})", 
                contact.getFirstName() + " " + contact.getLastName(),
                contact.getUpdatedAt()
            );
