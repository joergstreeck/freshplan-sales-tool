...
public class CustomerRepository implements PanacheRepositoryBase<Customer, UUID> {
    ...
    public List<Customer> findAllActive(Page page) {
        return find("isDeleted = false", Sort.by("companyName").ascending())
                .page(page)
                .list();
    }
    ...
    public List<Customer> findDeleted(Page page) {
        return find("isDeleted = true", Sort.by("deletedAt").descending())
                .page(page)
                .list();
    }
    ...
    public Optional<Customer> findByCustomerNumber(String customerNumber) {
        ...
        return find("customerNumber = ?1 AND isDeleted = false", customerNumber)
                .firstResultOptional();
    }
    ...
    /**
     * Full-text search in company name and trading name.
     * Uses LIKE with LOWER() instead of similarity() due to missing pg_trgm.
     */
    ...
    public List<Customer> findByStatusIn(List<CustomerStatus> statuses, Page page) {
        ...
        return find("status IN ?1 AND isDeleted = false", statuses)
                .page(page)
                .list();
    }
    ...
    public List<Customer> findByLifecycleStage(CustomerLifecycleStage stage, Page page) {
        return find("lifecycleStage = ?1 AND isDeleted = false", stage)
                .page(page)
                .list();
    }
    ...
    public List<Customer> findByIndustry(Industry industry, Page page) {
        return find("industry = ?1 AND isDeleted = false", industry)
                .page(page)
                .list();
    }
    ...
    public List<Customer> findChildren(UUID parentId) {
        return find("parentCustomer.id = ?1 AND isDeleted = false", parentId)
                .list();
    }
    ...
    public List<Customer> findRootCustomers(Page page) {
        return find("parentCustomer IS NULL AND isDeleted = false")
                .page(page)
                .list();
    }
    ...
    public List<Customer> findAtRisk(int minRiskScore, Page page) {
        return find("riskScore >= ?1 AND isDeleted = false", minRiskScore)
                .page(page)
                .list();
    }
    ...
    public List<Customer> findByExpectedVolumeRange(
        ...
        if (minVolume != null && maxVolume != null) {
            ...
            return find("expectedAnnualVolume >= ?1 AND isDeleted = false", minVolume)
                    .page(page)
                    .list();
        } else if (maxVolume != null) {
            return find("expectedAnnualVolume <= ?1 AND isDeleted = false", maxVolume)
                    .page(page)
                    .list();
        }
    ...
    /**
     * Find potential duplicates by company name similarity.
     * Uses LIKE-based search instead of similarity() due to missing pg_trgm.
     */
    ...
    public List<Customer> findRecentlyCreated(int days, Page page) {
        LocalDateTime cutoffDate = LocalDateTime.now().minusDays(days);
        return find("createdAt >= ?1 AND isDeleted = false", cutoffDate)
                .page(page)
                .list();
    }
    ...
    public List<Customer> findRecentlyUpdated(int days, Page page) {
        LocalDateTime cutoffDate = LocalDateTime.now().minusDays(days);
        return find("updatedAt >= ?1 AND isDeleted = false", cutoffDate)
                .page(page)
                .list();
    }
    ...
    /**
     * Get next customer number for CustomerNumberGenerator.
     * Format: KD-YYYY-XXXXX
     */
