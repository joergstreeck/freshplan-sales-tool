 ...
 * 
 * This service encapsulates the business logic for customer management,
 * providing a clean API for customer operations while handling validation,
...
public class CustomerService {
    ...
    public CustomerResponse createCustomer(@Valid CreateCustomerRequest request, String createdBy) {
        ...
        List<Customer> potentialDuplicates = customerRepository
                .findPotentialDuplicates(request.companyName());
        ...
        createTimelineEvent(customer, "CUSTOMER_CREATED", 
            "Kunde erstellt mit Nummer: " + customerNumber, createdBy, ImportanceLevel.HIGH);
    ...
    public void deleteCustomer(UUID id, String deletedBy, String reason) {
        ...
        createTimelineEvent(customer, "CUSTOMER_DELETED", 
            "Kunde archiviert. Grund: " + reason, deletedBy, ImportanceLevel.HIGH);
    }
    ...
    public CustomerResponse restoreCustomer(UUID id, String restoredBy) {
        ...
        createTimelineEvent(customer, "CUSTOMER_RESTORED", 
            "Kunde wiederhergestellt", restoredBy, ImportanceLevel.HIGH);
    ...
    public CustomerResponse addChildCustomer(UUID parentId, UUID childId, String updatedBy) {
        Customer parent = customerRepository.findByIdActive(parentId)
                .orElseThrow(() -> new CustomerNotFoundException("Parent customer not found", parentId));
        
        Customer child = customerRepository.findByIdActive(childId)
    ...
    public CustomerResponse mergeCustomers(UUID targetId, UUID sourceId, String mergedBy) {
        Customer target = customerRepository.findByIdActive(targetId)
                .orElseThrow(() -> new CustomerNotFoundException("Target customer not found", targetId));
        
        Customer source = customerRepository.findByIdActive(sourceId)
                .orElseThrow(() -> new CustomerNotFoundException("Source customer not found", sourceId));
     ...
     */
    public CustomerResponse changeStatus(UUID customerId, CustomerStatus newStatus, String updatedBy) {
        Customer customer = customerRepository.findByIdActive(customerId)
                .orElseThrow(() -> new CustomerNotFoundException(customerId));
        ...
        // Create timeline event
        createTimelineEvent(customer, "STATUS_CHANGE", 
            "Status geändert von " + oldStatus + " zu " + newStatus, updatedBy);
    ...
    private CustomerResponse mapToResponse(Customer customer) {
        ...
        return new CustomerResponse(
                ...
                customer.getParentCustomer() != null ? 
                        customer.getParentCustomer().getId().toString() : null,
     ...
     */
    private void createTimelineEvent(Customer customer, String eventType, 
    ...
    private EventCategory mapEventTypeToCategory(String eventType) {
        return switch (eventType) {
            case "CUSTOMER_CREATED", "CUSTOMER_DELETED", "CUSTOMER_RESTORED" -> 
                EventCategory.SYSTEM;
            case "STATUS_CHANGE", "LIFECYCLE_CHANGE" -> 
                EventCategory.STATUS_UPDATE;
            case "CONTACT_ADDED", "LOCATION_ADDED" -> 
