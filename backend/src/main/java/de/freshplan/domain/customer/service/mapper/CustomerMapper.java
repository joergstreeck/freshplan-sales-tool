 ...
 * Handles all transformations with proper validation and error handling.
 * 
...
public class CustomerMapper {
     ...
     * Includes all customer data and hierarchy information.
     * 
    ...
    /**
     * Maps a Customer entity to a minimal CustomerResponse for list views.
    ...
    /**
     * Maps a list of Customer entities to minimal CustomerResponse DTOs.
     * Optimized for list views with large datasets.
     * 
     * @param customers The customer entities
    ...
    /**
     * Maps a CreateCustomerRequest to a new Customer entity.
     * Sets all provided fields and applies defaults where needed.
     * 
    ...
    public Customer toEntity(CreateCustomerRequest request, String customerNumber, String createdBy) {
        ...
        if (request.parentCustomerId() != null && !request.parentCustomerId().isBlank()) {
            try {
                ...
                throw new CustomerMapperException("Invalid parent customer ID format: " + request.parentCustomerId());
            }
        }
        
        // Status & Lifecycle with defaults
        customer.setStatus(request.status() != null ? 
                request.status() : de.freshplan.domain.customer.entity.CustomerStatus.LEAD);
        customer.setLifecycleStage(request.lifecycleStage() != null ? 
                request.lifecycleStage() : de.freshplan.domain.customer.entity.CustomerLifecycleStage.ACQUISITION);
     ...
     * Only updates fields that are not null in the request (partial updates).
     * 
    ...
    public void updateEntity(Customer customer, UpdateCustomerRequest request, String updatedBy) {
        ...
        if (request.parentCustomerId() != null) {
            if (request.parentCustomerId().isBlank()) {
                ...
                customer.setHierarchyType(de.freshplan.domain.customer.entity.CustomerHierarchyType.STANDALONE);
            } else {
                try {
                    UUID parentId = UUID.fromString(request.parentCustomerId());
                    Optional<Customer> parent = customerRepository.findByIdActive(parentId);
    ...
    public CustomerListResponse toListResponse(
        ...
        List<CustomerResponse> customerResponses = useMinimal ? 
                toMinimalResponseList(customers) : 
                toResponseList(customers);
        
        return CustomerListResponse.of(customerResponses, page, size, totalElements);
    }
    
    /**
     * Maps Customer entities to CustomerListResponse with minimal data.
     * Optimized for large lists and better performance.
     * 
     * @param customers The customer entities
     * @param page Current page number
     * @param size Page size
