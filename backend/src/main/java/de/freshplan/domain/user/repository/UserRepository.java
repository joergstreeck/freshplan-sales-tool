 ...
 * 
 * This repository provides data access operations for User entities
 * following the repository pattern. It extends PanacheRepositoryBase
 * to leverage Quarkus Panache's active record pattern benefits while
...
public class UserRepository implements PanacheRepositoryBase<User, UUID> {
    ...
    public List<User> search(String searchTerm, Page page) {
        if (searchTerm == null || searchTerm.isBlank()) {
            return findAll(Sort.by("lastName", "firstName"))
                    .page(page)
                    .list();
        }
        
        String pattern = "%" + searchTerm.toLowerCase() + "%";
        return find("LOWER(username) LIKE ?1 OR LOWER(firstName) LIKE ?1 OR " +
                   "LOWER(lastName) LIKE ?1 OR LOWER(email) LIKE ?1", pattern)
                .page(page)
                .list();
    }
}