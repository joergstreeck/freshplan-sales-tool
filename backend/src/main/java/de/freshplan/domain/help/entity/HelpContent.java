 ...
 * 
 * Diese Entität speichert alle Arten von Hilfe-Inhalten:
 * - Tooltips und Erklärungen
 * - Feature Tours und Tutorials
 * - FAQ Inhalte
 * - Video-Links und Schritt-für-Schritt Anleitungen
 */
@Entity
...
public class HelpContent extends PanacheEntityBase {
    ...
    public static List<HelpContent> findForUser(String feature, String userLevel, List<String> userRoles) {
        List<HelpContent> results = find("feature = ?1 AND isActive = true AND " +
                   "(targetUserLevel = 'BEGINNER' OR " + 
                   "(targetUserLevel = 'INTERMEDIATE' AND ?2 != 'BEGINNER') OR " +
                   "(targetUserLevel = 'EXPERT' AND ?2 = 'EXPERT')) " +
                   "ORDER BY priority ASC", 
                   feature, userLevel.toUpperCase()).list();
        
        return results.stream()
               .filter(content -> {
                   return content.targetRoles == null || content.targetRoles.isEmpty() ||
