 ...
 * 
 * <p>Handles side effects of help content operations asynchronously
 * to maintain pure CQRS separation and improve performance.
 * 
 * <p>This handler processes events like:
 * - View count updates when content is viewed
 * - Analytics tracking for usage patterns
 * - Caching updates for performance optimization
 * 
...
public class HelpContentEventHandler {
    ...
    @Inject
    HelpContentCommandService commandService;
     ...
     * 
     * This handler executes in a separate transaction from the original
     * query operation, ensuring that side effects don't impact query performance
     * or cause the main operation to fail.
     *
    ...
    public void onHelpContentViewed(@ObservesAsync HelpContentViewedEvent event) {
        LOG.debug("Processing HelpContentViewedEvent for content: {} by user: {}", 
            event.helpContentId(), event.userId());
        ...
        try {
            ...
            if (event.originalRequest() != null && event.detectedStruggle() != null) {
                ...
                analyticsService.trackHelpRequest(
                    ...
                    contentForAnalytics, 
                    event.detectedStruggle()
                );
            }

            LOG.info("Successfully processed view event for help content: {} (user: {})", 
                event.helpContentId(), event.userId());

        } catch (Exception e) {
            LOG.error("Failed to process HelpContentViewedEvent for content: {} by user: {}", 
                event.helpContentId(), event.userId(), e);
     ...
     * 
     * Use this when view count updates must succeed within the same
     * transaction as the query operation.
     *
     * @param event The help content viewed event
     */
    @Transactional
    public void onHelpContentViewedSync(@Observes HelpContentViewedEvent event) {
        LOG.debug("Processing synchronous HelpContentViewedEvent for content: {}", 
            event.helpContentId());
        ...
        try {
            ...
            LOG.error("Critical: Failed to update view count synchronously for content: {}", 
                event.helpContentId(), e);
     ...
     * 
     * This could be triggered by scheduled jobs or admin operations.
     */
    public void onHelpContentBulkOperation(@ObservesAsync HelpContentBulkEvent event) {
        LOG.debug("Processing bulk operation: {} for {} items", 
            event.operation(), event.contentIds().size());
    ...
    public record HelpContentBulkEvent(
        ...
        java.util.List<java.util.UUID> contentIds
    ) {
        public enum BulkOperation {
            CACHE_WARM, CLEANUP_VIEWS, RECALCULATE_STATS
        }
    }
}