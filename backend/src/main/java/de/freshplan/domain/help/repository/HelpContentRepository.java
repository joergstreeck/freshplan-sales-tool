 ...
 *
 * <p>Bietet optimierte Queries für verschiedene Help-Szenarien: - Feature-spezifische Hilfe -
 * User-Level angepasste Inhalte - Analytics und Tracking
 */
@ApplicationScoped
public class HelpContentRepository implements PanacheRepositoryBase<HelpContent, UUID> {
  ...
  public List<HelpContent> findActiveByFeature(String feature) {
    return list("feature = ?1 AND isActive = true ORDER BY priority ASC, createdAt DESC", feature);
  }
  ...
  public List<HelpContent> findByFeatureAndType(String feature, HelpType helpType) {
    return list(
        "feature = ?1 AND helpType = ?2 AND isActive = true ORDER BY priority ASC",
        feature,
        helpType);
  }

  /** Findet den besten Tooltip für ein Feature basierend auf User-Level */
  public Optional<HelpContent> findBestTooltip(
      String feature, String userLevel, List<String> userRoles) {
    List<HelpContent> tooltips =
        list(
  ...
  public List<HelpContent> findForUserLevel(
      String feature, String userLevel, List<String> userRoles) {
  ...
  public List<HelpContent> findMostHelpful(int limit) {
    return find("isActive = true AND (helpfulCount + notHelpfulCount) >= 5 "
  ...
  public List<HelpContent> findNeedingImprovement(int limit) {
    return find("isActive = true AND (helpfulCount + notHelpfulCount) >= 10 "
  ...
  public List<HelpContent> searchContent(
  ...
  public long getTotalViews() {
    Long result =
        find("SELECT SUM(viewCount) FROM HelpContent WHERE isActive = true")
            .project(Long.class)
            .singleResult();
  ...
  public long getTotalFeedback() {
    Long result =
        find("SELECT SUM(helpfulCount + notHelpfulCount) FROM HelpContent WHERE isActive = true")
            .project(Long.class)
            .singleResult();
    return result != null ? result : 0L;
  }
  ...
  public double getOverallHelpfulnessRate() {
    List<Object[]> result =
        find("SELECT SUM(helpfulCount), SUM(notHelpfulCount) FROM HelpContent "
  ...
  public List<String> getFeaturesWithoutType(HelpType helpType) {
    ...
    return allFeatures.stream().filter(feature -> !featuresWithType.contains(feature)).toList();
  }
}
