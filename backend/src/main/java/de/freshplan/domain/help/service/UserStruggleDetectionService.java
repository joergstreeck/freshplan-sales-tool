 ...
 * 
 * Analysiert User-Verhalten und erkennt:
 * - Wiederholte fehlgeschlagene Versuche
 * - Hektische Navigation
 * - Lange Pausen nach dem Start
...
public class UserStruggleDetectionService {
    ...
    public UserStruggle detectStruggle(String userId, String feature, Map<String, Object> context) {
        ...
        if (detectedType != null) {
            ...
            LOG.info("Struggle detected for user {}: {} (severity: {}) in feature: {}", 
                    userId, detectedType, severity, feature);
     ...
     */
    private UserStruggle.StruggleType analyzeStrugglePatterns(UserBehaviorSession session, String feature) {
    ...
    private boolean hasRepeatedFailures(UserBehaviorSession session, String feature) {
        ...
        long failureCount = recentActions.stream()
    ...
    private boolean hasRapidNavigation(UserBehaviorSession session) {
        ...
        long uniquePages = recentActions.stream()
    ...
    private boolean hasAbandonedWorkflows(UserBehaviorSession session, String feature) {
        ...
        long abandonedCount = recentActions.stream()
            .filter(action -> action.feature.equals(feature))
    ...
    private boolean hasComplexFormStruggle(UserBehaviorSession session, String feature) {
        ...
        long formFieldChanges = recentActions.stream()
            .filter(action -> action.feature.equals(feature))
            .filter(action -> isFormFieldAction(action))
            .count();

        boolean hasSubmit = recentActions.stream()
    ...
    private List<UserStruggle.Suggestion> generateSuggestions(
        ...
        return switch (type) {
            case REPEATED_FAILED_ATTEMPTS -> List.of(
            ...
            case RAPID_NAVIGATION_CHANGES -> List.of(
            ...
            case LONG_IDLE_AFTER_START -> List.of(
            ...
            case ABANDONED_WORKFLOW_PATTERN -> List.of(
            ...
            case COMPLEX_FORM_STRUGGLE -> List.of(
    ...
    private boolean hasCorrespondingCompletion(List<UserAction> actions, UserAction startAction) {
        String expectedCompletion = startAction.context.get("action").toString()
                .replace("_start", "_complete");
    ...
    private static class UserBehaviorSession {
        ...
        public List<UserAction> getRecentActions(Duration window) {
            ...
            return actions.stream()
                    .filter(action -> action.timestamp.isAfter(cutoff))
                    .toList();
        }
