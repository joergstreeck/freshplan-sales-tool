...
/**
 * Idempotenter Event-Listener für Dashboard-Updates. KEIN AFTER_COMMIT hier - Listener laufen
 * außerhalb der DB-Transaktion! Stattdessen: Deduplizierung via Caffeine Cache.
 *
 * <p>Sprint 2.1.1 P0 HOTFIX - Idempotenz ohne AFTER_COMMIT in Listenern
 */
@ApplicationScoped
...
public class DashboardEventListener {
  ...
  @Inject SalesCockpitService cockpitService;
  ...
  /**
   * Verarbeitet Dashboard-Events von PostgreSQL NOTIFY. Events kommen bereits committed an
   * (LISTEN/NOTIFY läuft nach COMMIT).
   */
  @Transactional
    ...
    try {
      ...
      if (eventsProcessed.get() % CACHE_CHECK_INTERVAL == 0) {
        ...
        if (cacheSize > CACHE_WARNING_THRESHOLD) {
          ...
          Log.warnf(
              "Dedupe cache approaching limit: %d / %d entries (%.1f%%)",
  ...
  /**
   * Test-Helper: Direkte Event-Verarbeitung für Tests. Public für Test-Zugriff aus anderen
   * Packages.
   */
  @Transactional
  public void handleEnvelopeForTest(String channel, JsonObject envelope) {
    ...
    EventNotification notification =
        new EventNotification(channel, envelope.getString("type", "unknown"), envelope);
