...
/**
 * Publisher für Dashboard-Events mit AFTER_COMMIT Pattern. Stellt sicher, dass Events nur nach
 * erfolgreicher Transaktion publiziert werden.
 *
 * <p>Sprint 2.1.1 P0 HOTFIX - AFTER_COMMIT nur in Publishern, nicht in Listenern!
 */
@ApplicationScoped
...
public class DashboardEventPublisher {
  ...
  @Inject PgNotifySender pgNotifySender;
  ...
  @ConfigProperty(
      name = "freshplan.security.allow-unauthenticated-publisher",
      defaultValue = "false")
  ...
  private static final int MAX_NOTIFY_PAYLOAD_SIZE =
      7680; // 7.5 KB Safety Limit (Postgres max ~8KB)
  ...
  void logSecurityBypass() {
    if (allowUnauthenticatedPublisher) {
      Log.warn(
          "⚠️ freshplan.security.allow-unauthenticated-publisher=true (intended only for tests!)");
    }
  }
        ...
        new Synchronization() {
          ...
          public void afterCompletion(int status) {
            if (status == Status.STATUS_COMMITTED) {
              ...
              Log.debugf(
                  "Transaction rolled back, not publishing event for lead: %s", event.leadId());
            }
          }
        });
  }
  ...
  /**
   * Robuste pg_notify Implementation via Hibernate Session. Prüft Payload-Größe gegen
   * Postgres-Limit (~8KB).
   */
  private void notifyPg(String channel, String payload) {
    ...
    if (byteLen > MAX_NOTIFY_PAYLOAD_SIZE) {
      Log.warnf(
          "NOTIFY payload too large (%d bytes) for channel %s - truncating data field",
          byteLen, channel);

      // Envelope beibehalten, nur data kürzen
      JsonObject envelope = new JsonObject(payload);
      JsonObject truncatedData =
          new JsonObject()
              .put("truncated", true)
  ...
  /**
   * Generiert deterministischen Idempotency-Key für Follow-ups. Für BATCH-Events: Zeitfenster auf
   * Minute runden + User/Count-basiert.
   */
    ...
    if ("BATCH".equalsIgnoreCase(followUpType)) {
      ...
      LocalDateTime windowStart =
          Objects.requireNonNull(
                  event.processedAt(), "processedAt must not be null for BATCH events")
              .withSecond(0)
              .withNano(0);
      ...
      leadIdPart =
          UUID.nameUUIDFromBytes(("BATCH|" + stamp).getBytes(StandardCharsets.UTF_8)).toString();
    } else {
      // Für einzelne Leads: Timestamp aus Event verwenden
      LocalDateTime when =
          Objects.requireNonNull(
              event.processedAt(), "processedAt must not be null for follow-up events");
      stamp = when.toString();
      leadIdPart =
          Objects.requireNonNull(
                  event.leadId(), "leadId must not be null for non-BATCH follow-up events")
              .toString();
    }
  ...
  private boolean canPublishEvent(String userId, String eventType) {
    ...
    boolean hasRole =
        securityContext.hasRole("MANAGER")
