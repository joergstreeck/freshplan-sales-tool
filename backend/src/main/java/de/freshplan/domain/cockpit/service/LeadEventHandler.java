...
/**
 * Event Handler für Lead-Events zur Dashboard-Integration. Leitet Lead-bezogene Events an das
 * Dashboard-Modul weiter.
 *
 * <p>Sprint 2.1.1 P0 HOTFIX - Integration Gap zwischen Lead-Management und Dashboard
 */
@ApplicationScoped
...
public class LeadEventHandler {
  ...
  public void onLeadStatusChanged(@Observes LeadStatusChangeEvent event) {
    Log.infof(
        "Lead status changed: %s from %s to %s",
  ...
  private void trackFollowUpMetrics(FollowUpProcessedEvent event) {
    ...
    Log.debugf(
        "Metrics tracking delegated to DashboardEventPublisher for follow-up: %s",
        event.followUpType());
  }
}
