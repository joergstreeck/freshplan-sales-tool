-- V122: Add Data Quality and Freshness Tracking fields to contacts table
-- Migration für Data Freshness Tracking Feature (TODO-80)

-- Add data quality score field (0-100)
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS data_quality_score INTEGER;

-- Add recommendations field for storing improvement suggestions
ALTER TABLE contacts ADD COLUMN IF NOT EXISTS data_quality_recommendations TEXT;

-- Add comments for documentation
COMMENT ON COLUMN contacts.data_quality_score IS 'Overall data quality score (0-100) calculated by DataHygieneService';
COMMENT ON COLUMN contacts.data_quality_recommendations IS 'Semicolon-separated list of recommendations for improving data quality';
...
CREATE INDEX IF NOT EXISTS idx_contacts_data_quality_low ON contacts(data_quality_score);
