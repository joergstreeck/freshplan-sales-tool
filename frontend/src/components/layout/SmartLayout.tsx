...
function detectContentType(children: React.ReactNode): ContentWidth {
  ...
  const countElements = (element: any): void => {
    ...
    if (element.type?.name === 'TextField' ||
        element.type?.name === 'FormControl') {
      textFieldCount++;
    }
    
    // Check für Grid-Container mit mehreren Cards
    if (element.type?.name === 'Grid' ||
        element.type?.name === 'Grid2') {
      gridCount++;
    }
