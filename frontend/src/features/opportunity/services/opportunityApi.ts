...
export const opportunityApi = {
  ...
  async getAll(filters?: PipelineFilters, page = 0, size = 50): Promise<OpportunityResponse[]> {
    ...
    const response = await httpClient.get(`${OPPORTUNITY_BASE_URL}?${params.toString()}`);
    return response.data;
  },

  async getById(id: string): Promise<OpportunityResponse> {
    const response = await httpClient.get(`${OPPORTUNITY_BASE_URL}/${id}`);
    return response.data;
  },

  async create(request: CreateOpportunityRequest): Promise<OpportunityResponse> {
    const response = await httpClient.post(OPPORTUNITY_BASE_URL, request);
    return response.data;
  },

  async update(id: string, request: UpdateOpportunityRequest): Promise<OpportunityResponse> {
    const response = await httpClient.put(`${OPPORTUNITY_BASE_URL}/${id}`, request);
    return response.data;
  },

  async delete(id: string): Promise<void> {
    await httpClient.delete(`${OPPORTUNITY_BASE_URL}/${id}`);
  },

  // Stage Management
  async changeStage(id: string, request: ChangeStageRequest): Promise<OpportunityResponse> {
    const response = await httpClient.patch(`${OPPORTUNITY_BASE_URL}/${id}/stage`, request);
    return response.data;
  },
