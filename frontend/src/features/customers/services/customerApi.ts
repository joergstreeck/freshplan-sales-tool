...
export class CustomerApi {
  ...
  async createDraft(request: CreateCustomerDraftRequest = {}): Promise<CustomerDraftResponse> {
    return apiClient.post<CustomerDraftResponse>(`${this.basePath}/drafts`, request, { retry: 2 });
  }
  ...
  async getDraft(draftId: string): Promise<CustomerDraftResponse> {
    return apiClient.get<CustomerDraftResponse>(`${this.basePath}/drafts/${draftId}`, { retry: 2 });
  }
  ...
  async updateDraft(
    ...
    return apiClient.put<CustomerDraftResponse>(`${this.basePath}/drafts/${draftId}`, request, {
      retry: 1,
    });
  }
  ...
  async finalizeDraft(
    ...
    return apiClient.post<Customer>(`${this.basePath}/drafts/${draftId}/finalize`, request, {
      retry: 2,
    });
  }
  ...
  async getUserDrafts(): Promise<CustomerDraftResponse[]> {
    return apiClient.get<CustomerDraftResponse[]>(`${this.basePath}/drafts`, { retry: 2 });
  }
  ...
  async searchCustomers(request: CustomerSearchRequest): Promise<CustomerSearchResponse> {
    return apiClient.get<CustomerSearchResponse>(this.basePath, {
      params: {
        page: request.page,
        size: request.size,
        sort: request.sort,
  ...
  async getCustomer(customerId: string): Promise<CustomerWithFields> {
    return apiClient.get<CustomerWithFields>(`${this.basePath}/${customerId}`, { retry: 2 });
  }
  ...
  async deactivateCustomer(customerId: string, reason?: string): Promise<void> {
    return apiClient.post<void>(`${this.basePath}/${customerId}/deactivate`, { reason });
  }
  ...
  async exportCustomers(request: CustomerExportRequest): Promise<Blob> {
    const response = await fetch(
      `${import.meta.env.VITE_API_URL || 'http://localhost:8080'}${this.basePath}/export`,
      {
        method: 'POST',
        headers: {
  ...
  async validateImport(file: File): Promise<{
    ...
    const response = await fetch(
      `${import.meta.env.VITE_API_URL || 'http://localhost:8080'}${this.basePath}/import/validate`,
      {
        method: 'POST',
        headers: {
          Authorization: `Bearer ${sessionStorage.getItem('auth_token') || localStorage.getItem('auth_token')}`,
  ...
  async getRecentCustomers(limit: number = 10): Promise<CustomerListItem[]> {
    return apiClient.get<CustomerListItem[]>(`${this.basePath}/recent`, {
      params: { limit },
      retry: 2,
    });
  }
