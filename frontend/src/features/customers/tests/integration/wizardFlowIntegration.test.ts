...
describe('FC-005 Complete Wizard Flow Integration Tests', () => {
  describe('Single Customer Flow (chainCustomer = nein)', () => {
    it('should complete single customer onboarding workflow', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/schritt 3/i) || screen.getByText(/zusammenfassung/i)).toBeInTheDocument();
      });
  ...
  describe('Chain Customer Flow (chainCustomer = ja)', () => {
  ...
  describe('User Experience Integration', () => {
    ...
    it('should allow navigation back and forth between steps', async () => {
      ...
      const companyNameInput = screen.getByLabelText(/firmenname/i);
      expect((companyNameInput as HTMLInputElement).value).toBe(testData.validCustomerData.companyName);
    });
