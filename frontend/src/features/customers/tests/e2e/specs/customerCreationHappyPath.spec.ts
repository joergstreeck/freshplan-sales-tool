...
test.describe('FC-005 Customer Creation Happy Path', () => {
  ...
  test('should complete single customer onboarding successfully', async ({ page }) => {
    ...
    await test.step('Fill customer basic information', async () => {
      ...
      await expect(onboardingPage.companyNameField).toHaveValue(testCustomers.singleLocation.companyName);
      await expect(onboardingPage.industrySelect).toHaveValue(testCustomers.singleLocation.industry);
      await expect(onboardingPage.chainCustomerSelect).toHaveValue(testCustomers.singleLocation.chainCustomer);
    });
    ...
    await test.step('Verify hotel-specific fields are visible', async () => {
      ...
      await expect(onboardingPage.hotelStarsSelect).toHaveValue(testCustomers.singleLocation.hotelStars);
    });
  ...
  test('should handle extremely long company names gracefully', async ({ page }) => {
    const longCompanyName = 'A'.repeat(200) + ' Extremely Long Company Name GmbH & Co. KG mit sehr langem Namen für Test';
