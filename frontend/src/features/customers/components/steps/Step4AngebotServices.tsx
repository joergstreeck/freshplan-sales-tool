...
export const Step4AngebotServices: React.FC = () => {
  ...
  // Handlers
  const handleServiceChange = useCallback((field: string, value: any) => {
  ...
  }, [currentLocation, currentServices, saveLocationServices, applyToAll, locations, completedLocationIds, applyToAllRemaining]);
  
  const handleCopyFrom = useCallback((sourceLocationId: string) => {
    if (!currentLocation) return;
