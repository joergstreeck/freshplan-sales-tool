...
export const Step3Ansprechpartner: React.FC = () => {
  ...
  const contactDisplayName = useMemo(() => {
    ...
    if (customerData.salutation && customerData.salutation !== 'keine') {
      parts.push(
        customerData.salutation === 'herr'
          ? 'Herr'
          : customerData.salutation === 'frau'
            ? 'Frau'
            : ''
      );
    }
  ...
  return (
    <Box>
      ...
      <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
        Erfassen Sie Kontaktdaten für besseren Beziehungsaufbau. Dieser Schritt kann auch später
        ergänzt werden.
      </Typography>

      <Alert severity="info" sx={{ mb: 3 }}>
        <AlertTitle>Tipp für erfolgreichen Vertrieb</AlertTitle>
        Je mehr Sie über Ihre Ansprechpartner wissen, desto besser können Sie auf deren Bedürfnisse
        eingehen. Die zusätzlichen Felder helfen beim Beziehungsaufbau.
      </Alert>
          ...
          <Card variant="outlined" sx={{ mt: 2, bgcolor: 'grey.50' }}>
            <CardContent>
              ...
              {customerData.position && (
                <Typography variant="body2">{customerData.position}</Typography>
              )}
            </CardContent>
          </Card>
        )}
      ...
      <Card sx={{ bgcolor: 'primary.light', mt: 3 }}>
        <CardContent>
          ...
          <Typography variant="body2">
            Nach dem Speichern wird automatisch eine Aufgabe "Neukunde begrüßen" erstellt, damit der
            erste Kontakt nicht vergessen wird.
          </Typography>
        </CardContent>
      </Card>
    </Box>
