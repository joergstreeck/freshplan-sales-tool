...
vi.mock('../adaptive/AdaptiveFormContainer', () => ({
  AdaptiveFormContainer: ({ children }: any) => <div className="adaptive-form-container">{children}</div>
}));
...
describe('Step3MultiContactManagement', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockContacts = [];
  ...
  it('should display multiple contacts', () => {
    mockContacts = [
      ...
      createMockContact({ id: 'contact-2', salutation: 'Frau', firstName: 'Maria', lastName: 'Musterfrau', isPrimary: false }),
      createMockContact({ id: 'contact-3', salutation: 'Herr', firstName: 'Peter', lastName: 'Schmidt', isPrimary: false })
    ];
    mockStore.contacts = mockContacts;
    ...
    const papers = screen.getAllByRole('region').filter(el => 
      el.className.includes('MuiPaper-root')
    );
