...
export const SegmentedRevenueCalculator: React.FC<SegmentedRevenueCalculatorProps> = ({
  ...
  const totalLocations = locationsGermany + locationsAustria + locationsSwitzerland + locationsRestEU;
  
  // Debug-Ausgabe
  console.log('Filialanzahlen:', { locationsGermany, locationsAustria, locationsSwitzerland, locationsRestEU, totalLocations });
  
  const [segments, setSegments] = useState<HotelSegment[]>([
    { size: 'small', count: 0, services: { breakfast: false, alacarte: false, banquet: false, roomservice: false } },
    { size: 'medium', count: 0, services: { breakfast: false, alacarte: false, banquet: false, roomservice: false } },
    { size: 'large', count: 0, services: { breakfast: false, alacarte: false, banquet: false, roomservice: false } }
  ]);
  ...
  const getSizeLabel = (size: string): string => {
    switch (size) {
      case 'small': return 'Klein (<50 Zimmer)';
      case 'medium': return 'Mittel (50-120 Zimmer)';
      case 'large': return 'Groß (>120 Zimmer)';
      default: return size;
    }
  };
  ...
  return (
    ...
    >
      ...
      <Alert severity="info" sx={{ mb: 3 }}>
        Erfassen Sie grob Ihre Hotelstruktur. Die genaue Aufschlüsselung erfolgt später pro Standort.
      </Alert>
      ...
      {totalLocations > 0 && totalEnteredHotels > 0 && totalEnteredHotels !== totalLocations && (
        <Alert severity="warning" sx={{ mb: 2 }}>
          Sie haben {totalEnteredHotels} Hotels in der Kalkulation erfasst, aber {totalLocations} Filialen in Schritt 1 angegeben. 
      ...
      >
        💡 Diese Schätzung können Sie jederzeit anpassen. In Schritt 4 erfassen Sie die Details pro Standort.
      </Typography>
    </Paper>
  );
};