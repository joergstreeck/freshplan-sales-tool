...
describe('useDropdownWidth', () => {
  ...
  describe('Hook Funktionalität', () => {
    it('berechnet Breite basierend auf längster Option', () => {
      const { result } = renderHook(() => 
        useDropdownWidth({ options: sampleOptions })
      );
    ...
    it('begrenzt auf Maximum-Breite', () => {
      const veryLongOptions: FieldOption[] = [
        { 
          value: 'very_long', 
          label: 'Extrem langer Text der definitiv über die maximale Breite hinausgeht und gekürzt werden sollte' 
        }
      ];
  ...
  describe('Edge Cases', () => {
    it('behandelt undefined options', () => {
      const { result } = renderHook(() => 
        useDropdownWidth({ options: undefined })
      );
