...
export const customerBaseSchema = z.object({
  ...
  industry: z.enum(
    ['hotel', 'krankenhaus', 'seniorenresidenz', 'restaurant', 'betriebsrestaurant'],
    {
...
export const hotelFieldsSchema = z.object({
  ...
  restaurantSeats: optionalPositiveIntegerWithMaxSchema(
    9999,
    'Anzahl Sitzplätze darf maximal 9999 sein'
  ).describe('Restaurant Sitzplätze'),
  breakfastGuests: optionalPositiveIntegerSchema.describe(
    'Durchschnittliche Frühstücksgäste pro Tag'
  ),
  hasConferenceRooms: z.boolean().optional(),
  conferenceCapacity: optionalPositiveIntegerSchema.describe('Maximale Konferenz-Kapazität'),
});
...
export const hospitalFieldsSchema = z.object({
  ...
  patientMealsPerDay: positiveIntegerSchema.describe('Patientenmahlzeiten pro Tag'),
  staffMealsPerDay: optionalPositiveIntegerSchema.describe('Mitarbeitermahlzeiten pro Tag'),
...
export const seniorResidenceFieldsSchema = z.object({
  ...
  mealsPerResident: z.number().min(1).max(5).describe('Mahlzeiten pro Bewohner pro Tag'),
...
export const restaurantFieldsSchema = z.object({
  ...
  averageGuestsPerDay: positiveIntegerSchema.describe('Durchschnittliche Gäste pro Tag'),
...
export const companyRestaurantFieldsSchema = z.object({
  ...
  dailyLunchGuests: positiveIntegerSchema.describe('Durchschnittliche Mittagsgäste pro Tag'),
...
export type CustomerHospital = z.infer<typeof customerBaseSchema> &
  z.infer<typeof hospitalFieldsSchema>;
export type CustomerSeniorResidence = z.infer<typeof customerBaseSchema> &
  z.infer<typeof seniorResidenceFieldsSchema>;
export type CustomerRestaurant = z.infer<typeof customerBaseSchema> &
  z.infer<typeof restaurantFieldsSchema>;
export type CustomerCompanyRestaurant = z.infer<typeof customerBaseSchema> &
  z.infer<typeof companyRestaurantFieldsSchema>;
