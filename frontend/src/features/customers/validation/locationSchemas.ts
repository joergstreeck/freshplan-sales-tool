...
export const locationTypeEnum = z.enum(
  ['hauptstandort', 'filiale', 'aussenstelle', 'niederlassung', 'produktionsstaette'],
  {
...
export const hospitalLocationSchema = locationBaseSchema.extend({
  bedCount: positiveIntegerSchema.max(999, 'Anzahl Betten darf maximal 999 sein'),
  departments: z.array(z.string()).optional(),
  hasEmergency: z.boolean().optional(),
  kitchenCapacity: optionalPositiveIntegerSchema.describe('Küchenkapazität Mahlzeiten/Tag'),
});
...
export const seniorResidenceLocationSchema = locationBaseSchema.extend({
  residentCount: positiveIntegerSchema.max(999, 'Anzahl Bewohner darf maximal 999 sein'),
...
export const restaurantLocationSchema = locationBaseSchema.extend({
  seatCount: positiveIntegerSchema.max(999, 'Anzahl Sitzplätze darf maximal 999 sein'),
  outdoorSeats: optionalPositiveIntegerSchema,
  kitchenSize: optionalPositiveIntegerSchema.describe('Küchengröße in m²'),
...
export const companyRestaurantLocationSchema = locationBaseSchema.extend({
  ...
  servingLines: optionalPositiveIntegerSchema.describe('Anzahl Ausgabelinien'),
  kitchenCapacity: optionalPositiveIntegerSchema.describe('Max. Mahlzeiten pro Service'),
});
...
export const detailedLocationSchema = z.object({
  ...
  category: z
    .enum(['restaurant', 'cafeteria', 'kiosk', 'station', 'kitchen', 'storage', 'other'])
    .describe('Art der Detail-Location'),

  floor: z.string().max(20).optional().describe('Stockwerk/Ebene'),

  // Kapazitäten
  capacity: optionalPositiveIntegerSchema.describe('Kapazität (Personen/Betten/etc.)'),

  // Betriebszeiten
  operatingHours: z.string().max(100).optional().describe('z.B. "Mo-Fr 11:30-14:00"'),
