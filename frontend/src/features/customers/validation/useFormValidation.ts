...
export function useFormValidation<T extends FieldValues = FieldValues>(
  ...
  const { validationErrors, setValidationError, clearValidationError, clearAllValidationErrors } =
  ...
  const schema = useMemo(() => {
    ...
    switch (entityType) {
      case EntityType.CUSTOMER:
        return isDraft ? getCustomerDraftSchema(industry) : getCustomerSchema(industry);
  ...
  const validateField = realtimeValidation ? validateFieldDebounced : validateFieldImmediate;
