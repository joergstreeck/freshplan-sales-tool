...
describe('Customer Onboarding Store - Contact Integration', () => {
  beforeEach(() => {
    // Reset store state
    act(() => {
  ...
  describe('Contact Validation', () => {
    it('should validate required fields', async () => {
      ...
      expect(result.current.contactValidationErrors[contactId].firstName).toBe('Vorname ist erforderlich');
    });
    ...
    it('should validate email format', () => {
      ...
      expect(result.current.contactValidationErrors[contactId]?.email).toBe('Ungültige E-Mail-Adresse');
    });
