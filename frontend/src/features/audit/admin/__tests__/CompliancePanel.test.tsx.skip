...
describe('CompliancePanel', () => {
  ...
  it('should show issue severity with correct icons', () => {
    ...
    // Warning issue
    const warningIssue = screen
      .getByText('Fehlende Rechtsgrundlage für 5 Verarbeitungen')
      .closest('.MuiListItem-root');
    expect(within(warningIssue!).getByTestId('WarningIcon')).toBeInTheDocument();

    // Error issue
    const errorIssue = screen
      .getByText('Überfällige Löschung von 12 Datensätzen')
      .closest('.MuiListItem-root');
  ...
  it('should color code progress bars based on value', () => {
    ...
    const integrityBar = screen
      .getByText('Datenintegrität')
      .closest('.MuiBox-root')
    ...
    const retentionBar = screen
      .getByText('Retention Compliance')
      .closest('.MuiBox-root')
      ?.querySelector('.MuiLinearProgress-root');
