    ...
    <Box>
      {data.items.map((t:ThreadItem)=> (
        <Paper key={t.id} sx={{p:2, mb:1, borderRadius:'10px', boxShadow:'var(--shadow-sm)'}}>
          ...
          <Typography variant="body2" color="text.secondary">{new Date(t.lastMessageAt).toLocaleString()} • {t.territory}</Typography>
          <Chip size="small" label={t.channel} color={t.channel==='EMAIL'?'primary':'secondary'} sx={{ml:1}}/>
