...
/**
 * ABAC-Secured Query Service for Reports Data Access
 *
 * @see ../../grundlagen/SECURITY_GUIDELINES.md - ABAC Security Implementation
 * @see ../../grundlagen/PERFORMANCE_STANDARDS.md - Query Optimization Guidelines
 * @see ../../grundlagen/CODING_STANDARDS.md - Named Parameters SQL Injection Prevention
 *
 * This service provides secure, territory-scoped queries for all Reports endpoints.
 * All queries enforce ABAC (Attribute-Based Access Control) through ScopeContext
 * and use named parameters to prevent SQL injection attacks.
 *
 * Performance considerations:
 * - Cursor-based pagination for large result sets
 * - Index-optimized queries on territory and customer_id columns
 * - Fallback strategies for missing data sources
 *
 * @author Backend Team
 * @version 1.1
 * @since 2025-09-19
...
public class ReportsQuery {
  ...
  /** JPA Entity Manager for database access */
  @Inject EntityManager em;

  /** Security Scope Context for ABAC territory/chain filtering */
  @Inject ScopeContext scope;

  /**
   * Fetch Customer Analytics with ABAC Security and Pagination
   *
   * @param segment Customer segment filter (gastronomiebetriebe, direktkunden)
   * @param territory Territory filter (automatically scoped by ScopeContext)
   * @param seasonFrom Start date for seasonal analysis (YYYY-MM-DD)
   * @param seasonTo End date for seasonal analysis (YYYY-MM-DD)
   * @param renewalBefore Filter customers with renewals before this date
   * @param limit Maximum records (1-200, automatically clamped)
   * @param cursor Pagination cursor (customer ID for next page)
   * @return Map with "items" (customer list) and "nextCursor" (pagination)
  ...
  /**
   * Fetch Activity Statistics with Time-Range Filtering and ABAC Security
   *
   ...
   * @return Map with activity totals and daily breakdown
   * @see ../../grundlagen/PERFORMANCE_STANDARDS.md - Aggregation Query Optimization
   */
  ...
  /**
   * Fetch Cost Overview using existing CostStatistics Service
   *
   * @param costsService Injected CostStatistics service (existing 200+ LOC)
   * @param range Time range for cost analysis (week, month, quarter, year)
   * @return Map with cost totals and category breakdown
   * @see ../../grundlagen/API_STANDARDS.md - Service Integration Patterns
   */
  public Map<String,Object> fetchCostOverview(Object costsService, String range) {
    // Adapter to existing CostStatistics service
    // This delegates to the existing headless service without logic duplication
    Map<String,Double> totals = new LinkedHashMap<>();
    List<Map<String,Object>> breakdown = new ArrayList<>();

    // TODO: Real delegation to CostStatistics.calculateTotals(range)
    // Pattern: var stats = ((CostStatistics)costsService).calculateTotals(range);
    totals.put("total", 0.0);
    breakdown.add(Map.of("category","placeholder","amount",0.0));
