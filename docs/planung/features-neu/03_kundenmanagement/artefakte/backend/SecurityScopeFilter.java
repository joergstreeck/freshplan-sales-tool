  ...
  public void filter(ContainerRequestContext ctx) {
    ...
    if (auth != null && auth.startsWith("Bearer ")) {
      try {
        ...
        Object c = jwt.getClaim(CLAIM_CHAIN);
        if (c != null) chain = String.valueOf(c);
      } catch (ParseException e) {
        // ignore; may fallback to headers in dev
      }
    }
    if (ALLOW_HEADER_SCOPES && terrs.isEmpty()) {
      String h = ctx.getHeaderString("X-Territories");
      if (h != null) terrs.addAll(Arrays.asList(h.split(",")));
