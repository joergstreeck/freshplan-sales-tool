...
# Update: 26.07.2025 - Dynamische Feature-Suche
...
if [ -n "$MODULE" ] && [ "$MODULE" != "null" ]; then
    ...
    # NEU: Suche auch in FC-XXX-* Struktur
    if [ -z "$FOUND_DOCS" ]; then
        FC_DOC=$(find docs/features -type d -name "${FEATURE}-*" 2>/dev/null | head -1)
        if [ -n "$FC_DOC" ] && [ -f "$FC_DOC/README.md" ]; then
            FOUND_DOCS="$FC_DOC/README.md"
