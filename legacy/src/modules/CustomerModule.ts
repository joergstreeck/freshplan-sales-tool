...
import { getCustomerFormTemplate } from '../templates/customerForm';
import type { 
  CustomerData, 
  Industry, 
  ValidationRule,
...
export default class CustomerModule extends Module {
  ...
  async setup(): Promise<void> {
    ...
    // Insert customer form HTML into the tab
    const customerTab = this.dom.$('#tab-customer');
    if (customerTab) {
      customerTab.innerHTML = getCustomerFormTemplate();
    }
  ...
  bindEvents(): void {
    ...
    // Customer type selection (single/chain)
    this.on(document, 'change', 'input[name="customerTypeSelection"]', (e: Event) => {
      const input = e.target as HTMLInputElement;
      this.handleCustomerTypeSelectionChange(input.value);
    });

    // Customer status change (new/existing)
    this.on(document, 'change', '#customerStatus', (e: Event) => {
      const select = e.target as HTMLSelectElement;
      this.handleCustomerStatusChange(select.value);
    });
    ...
    this.on(document, 'change', '#paymentMethod', (e: Event) => {
      const select = e.target as HTMLSelectElement;
      this.handlePaymentMethodChange(select.value);
    });

    // Vending interest checkbox
    this.on(document, 'change', '#vendingInterest', (e: Event) => {
      const checkbox = e.target as HTMLInputElement;
      this.handleVendingInterestChange(checkbox.checked);
    });
  ...
  private setupValidationRules(): void {
    ...
    this.validationRules.set('customerStatus', { required: true });
    ...
    this.validationRules.set('annualVolume', { required: true });
    this.validationRules.set('paymentMethod', { required: true });
  }
  ...
  /**
   * Handle customer type selection change (single/chain)
   */
  private handleCustomerTypeSelectionChange(type: string): void {
    const chainFields = this.dom.$('#chainFields');
    const chainField = this.dom.$('.chain-field');
    
    if (type === 'chain') {
      if (chainFields) this.dom.removeClass(chainFields, 'hidden');
      if (chainField) this.dom.removeClass(chainField, 'hidden');
      
      // Update store
      useStore.getState().setCustomerType('chain');
    } else {
      if (chainFields) this.dom.addClass(chainFields, 'hidden');
      if (chainField) this.dom.addClass(chainField, 'hidden');
      
      // Update store
      useStore.getState().setCustomerType('single');
    }
  }

  /**
   * Handle customer status change (new/existing)
   */
  private handleCustomerStatusChange(status: string): void {
    const alert = this.dom.$('#newCustomerAlert');
    const paymentMethod = (this.dom.$('#paymentMethod') as HTMLSelectElement)?.value;
    
    if (status === 'neukunde' && paymentMethod === 'rechnung') {
      if (alert) this.dom.removeClass(alert, 'hidden');
    } else {
      if (alert) this.dom.addClass(alert, 'hidden');
    }
    
    // Update store
    useStore.getState().setCustomerData({ customerStatus: status as 'neukunde' | 'bestandskunde' });
  }
  ...
  private handlePaymentMethodChange(method: string): void {
    const customerStatus = (this.dom.$('#customerStatus') as HTMLSelectElement)?.value;
    const alert = this.dom.$('#newCustomerAlert');
    
    if (customerStatus === 'neukunde' && method === 'rechnung') {
      if (alert) this.dom.removeClass(alert, 'hidden');
    } else {
      if (alert) this.dom.addClass(alert, 'hidden');
    }
    ...
    // Update store
    useStore.getState().setCustomerData({ paymentMethod: method as 'vorkasse' | 'bar' | 'rechnung' });
  }

  /**
   * Handle vending interest change
   */
  private handleVendingInterestChange(interested: boolean): void {
    const vendingFields = this.dom.$('#vendingFields');
    
    if (interested) {
      if (vendingFields) this.dom.removeClass(vendingFields, 'hidden');
    } else {
      if (vendingFields) this.dom.addClass(vendingFields, 'hidden');
    }
    
    // Update store
    useStore.getState().setCustomerData({ vendingInterest: interested });
  }
