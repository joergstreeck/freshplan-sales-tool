# Customer Module Code Samples - JavaScript Version

Dies sind die wichtigsten Code-Ausschnitte aus unserer CustomerModuleV2 Implementierung, konvertiert von TypeScript zu JavaScript für bessere Lesbarkeit.
...
## 10. Die konkrete Fehlerstelle

```javascript
// In FreshPlanApp.ts
if (useV2) {
  console.log('🔄 Using CustomerModuleV2 (Phase 2)');
  this.registerModule('customer', new CustomerModuleV2());
} else {
  this.registerModule('customer', new CustomerModule());
}

// Das Module wird registriert, aber später:
// 1. Save-Button click → NICHTS passiert
// 2. Clear-Button click → NICHTS passiert
// 3. Form-Events → NICHTS passiert
```
...
bindEvents() {
  const saveBtn = this.dom.$('.header-btn-save');
  console.log('Save button found:', saveBtn); // ✓ Button wird gefunden
  
  if (saveBtn) {
    // Direkter Event-Listener (funktioniert!)
    saveBtn.addEventListener('click', () => {
      console.log('Direct listener works!');
    });
    
    // Module.on() Method (funktioniert NICHT!)
    this.on(saveBtn, 'click', async () => {
      console.log('Module.on listener - never called!');
      await this.handleSave();
    });
  }
}
